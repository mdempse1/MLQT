...
| B25 | **A parse error ends up owned by two classes** | found while doing B13 | ⭐⭐ | M | The load path distributes a file's parser errors to whichever extracted class's line range contains them, which for a nested class is the *package*. Then the first check of that class calls `EnsureParsed` on its own source, finds no errors recorded against it, and records the ones it re-derives — so the same problem is now attached to two nodes, and every surface that walks the graph reports it twice. `ModelDefinition.EnsureParsed` guards against *overwriting* the load-time errors (deliberately: the file-level message is the better one), but not against a second node acquiring its own copy. Needs a decision about which node owns a parse error — the class the text belongs to, presumably — applied on both paths, and it touches the GUI's issue list as well as MCP. Deliberately not folded into B13, which is about what a finding *says*, not about how many of them exist. |
