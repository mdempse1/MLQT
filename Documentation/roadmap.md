...
| B12 | **A `replaceable` nested class is checked twice** | found while doing B5 | ⭐⭐ | M | A nested class carrying a `replaceable`/`redeclare` prefix has its own node *and* is walked again as part of its parent, so every finding in it is reported twice — same rule, same element, same fingerprint, from two different calls, which is why the per-call de-duplication does not catch it. Rule-agnostic (a description finding doubles as readily as a unit one) and long-standing: 150 duplicated findings on MSL, and duplicate rows in the app. Stopping the parent's walk is the fix, but the guard that would stop it is also what lets a single-class check see such a class at all. |
...
**B1–B5 and B8 are done.** B8–B11 (added 2026-09-03) came out of asking how the SARIF work would be tested,
and they change the claim that phase 4 is closed: the output has never been validated, and the one
consumer it was written for renders almost none of what it carries. **B9 is next** — small, and a defect in
shipped output rather than a gap in coverage — with B10 decided alongside it, since what SARIF
carries for accepted debt changes what the smoke fixture should assert. B11 is run once B8–B10 land, as
