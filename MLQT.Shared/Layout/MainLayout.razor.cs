...
public partial class MainLayout : IDisposable
{
    ...
    {
        var graph = LibraryDataService.CombinedGraph;
        var effectiveSettings = styleSettings;
        ...
        var formatting = effectiveSettings.ToFormattingOptions();

        // Build list of files to process with their model nodes
        var filesToProcess = new List<(string FilePath, List<ModelNode> Models, ModelNode Owner)>();
        foreach (var filePath in changedFilePaths)
        {
            ...
            // Excluding the whole file is the safe reading of the exclusion — never reformat a model
            // the user has opted out.
            //
            // Both mechanisms, through the one method that knows about both. This asked the name list
            // alone, so __MLQT(format=false) — the rename-safe successor the documentation steers
            // people to — was honoured by Format All Files and ignored by this path, which is the one
            // that runs at startup and after every VCS operation. The class it was written on was
            // reordered in the working copy on the next pull.
            if (modelNodes.Any(m => FormattingExclusion.Excludes(m, effectiveSettings)))
            {
                Debug("MainLayout", $"Skipping {filePath}: it holds a model excluded from formatting");
                continue;
            }

            // The file owner is the topmost model stored in the file: it has no parent, or its parent
            // lives in another file. Only its within clause describes the file, and only when the file
            // itself carries none.
            ...
            filesToProcess.Add((filePath, modelNodes, owner));
        }
        ...
        await Task.Run(() =>
        {
            Parallel.ForEach(filesToProcess, fileEntry =>
            {
                try
                {
                    // Render the file from its own text on disk, never by concatenating the stored code
                    // of the models it holds. A model's ModelicaCode is not a file slice: a package has
                    // had its inline standalone children trimmed out of it (PackageCodeTrimmer), nested
                    // classes have their own nodes but live inside their parent's source, and a model
                    // that has been through a full library save carries a within clause while a freshly
                    // loaded one does not. Reassembling from those pieces duplicated the within clause
                    // and re-emitted every nested class as a top-level sibling.
                    ...
                    var formatted = ModelicaPackageSaver.RenderFileSource(
                        fileSource, fileEntry.Owner.ParentModelName,
                        formatting,
                        out var parserErrors);

                    // Reformatting invalid Modelica produces unreliable output, and this path overwrites
                    // the file in place. Leave a file we cannot parse exactly as the user left it — the
                    // style check reports the syntax error, which is the actionable result.
                    if (parserErrors.Count > 0)
                    {
                        Warn("MainLayout", $"Not formatting {fileEntry.FilePath}: {parserErrors.Count} syntax error(s), first at line {parserErrors[0].Line}: {parserErrors[0].Message}");
                        return;
                    }

                    formattedFiles[fileEntry.FilePath] = formatted.TrimEnd() + "\n";

                    // Bring each model's stored code up to date with what is about to be written, so
                    // style checking and the code viewer see the formatted source without waiting for a
                    // reload. Stored without a within clause, which is the representation the rest of
                    // the graph expects — keeping one would shift every finding's line number by one.
