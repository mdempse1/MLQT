...
| B125 | **Photino is noticeably slower than MAUI on some operations, after B121** | phase 7b-3 2026-09-08 | ⭐⭐⭐ | M | Reported from real use on Windows and Linux once the settings migration landed: nothing is broken — every feature works and conformance is clean — but **some steps are a lot slower than the MAUI build was**. This is what is left after B121 silenced Photino's per-message stdout logging, which the same reporter confirmed "made it a lot better"; so it is a second, smaller cause, not the same one again. Nothing here says which steps, because that is the first thing to establish rather than guess — the last time this was guessed at, the answer was logging and the hypothesis had been rendering. **Owned by 7b-5**, alongside the Windows conformance run, because a host that passes every probe and feels slow is exactly the failure conformance cannot see. |
