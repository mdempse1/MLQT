...
| B132 | **The Photino window had no taskbar icon** | phase 7b-5 2026-09-08 | ⭐⭐ | S | `ApplicationIcon` puts the icon on the `.exe` — Explorer and Alt-Tab were right — but **Photino creates its own native window and that window has no icon unless one is set on it**, so the taskbar entry stayed generic. Fixed by setting it through `SetIconFile`, per platform. **Reported still generic**, with the title bar and Explorer correct: three places, three sources. Measured rather than guessed — the window carried a correct 16x16 `ICON_SMALL` and 32x32 `ICON_BIG`, and the `.ico` holds 16/24/32/48/256, so neither the window nor the file was ever the problem. Two further changes, each justified on its own: an explicit **Application User Model ID** (`MLQTProject.MLQT`), because a taskbar button is grouped by AUMID and an application declaring none inherits whatever the shell derives from the process — `dotnet.exe` under `dotnet run`, or a cached entry for a path it saw many times while the executable had no icon at all; and `WindowIcon.Apply`, which re-attaches the icon **after** the window exists (a button keeps what it had when it appeared) at the **DPI-scaled size** (the taskbar wants 40 at 125% and was being handed 32 to stretch; it now gets 40x40 and 20x20, measured). Assets in `Branding/`. |
