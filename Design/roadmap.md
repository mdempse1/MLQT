...
| B154 | **A journey fails about one full-suite run in four, always the same one** | doc screenshots 2026-09-11 | ⭐⭐ | S | `TabNavigationJourney.MovingThroughEveryTabAndBack_RaisesNothing` times out after 30s clicking a tab — Playwright reports the element "visible, enabled and stable" and the click never completes. Seen **twice in about seven full-suite runs**, and **not once in six runs of that journey on its own** (3 of the class alone, then 2 full suites and the run before it), so it is the suite around it rather than the journey itself: every journey shares one host and one browser through the collection fixture, and the failing click is the fifth tab of a page another journey has already been driving. **It was noticed because a new class joined that collection — the screenshot generator — and the first failures came afterwards, so that was the obvious suspect. It has been ruled out**: with the generator excluded by filter the suite still failed once in three runs, the same test the same way. The rate is about one full-suite run in three or four either way, and **six runs of that journey on its own have never failed**. So it is the suite around it: every journey shares one host and one browser, and by the time this one runs the page has been driven by a dozen others. (The generator's `LibraryFixture` was initialising a git repository on every journey run even when it did nothing, which is fixed regardless — it is lazy now.) **The next step is to run the suite in a loop and read the trace**, which is possible now that B148 records one. **Why it matters more than the rate suggests**: the journeys run on two platforms on every push, and the nightly WebKit job has never run at all. A flake that costs one run in four turns a red build into something people re-run rather than read, which is how a real failure gets missed. |
