...
| B132 | **The Photino window had no taskbar icon** | phase 7b-5 2026-09-08 | ⭐ | S | The other difference the checklist found. `ApplicationIcon` puts the icon on the `.exe` — Explorer and Alt-Tab were right — but **Photino creates its own native window and that window has no icon unless one is set on it**, so the taskbar entry stayed generic and the application looked unbranded beside the MAUI build. **Fixed:** the icon is set at runtime through `SetIconFile`, per platform (Windows `.ico`, GTK PNG), with both copied beside the executable because that API takes a path rather than a resource. A missing file logs and is skipped. Assets are in `Branding/`, from the designer, with a README covering what each is for. |
