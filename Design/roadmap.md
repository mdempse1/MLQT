...
**All three are closed**, the guard first and watched failing on the other two before either was
fixed; widening it turned the four wrong labels into eight.
...
**Phase 7a was planned out in detail on 2026-09-07** and its design note restructured around a new policy: **component logic lives in a `.razor.cs` code-behind partial, not an `@code { }` block**, recorded in [CODING_GUIDELINES.md](../CODING_GUIDELINES.md) and adapted from the workspace-wide `Claytex.Net` guidelines. The measurement that motivated it: `MLQT.Shared` is 16,652 lines of `.razor` across 39 components, **11,935 of them inside `@code` blocks and zero `.razor.cs` files** — 71% of the UI source unreachable by any test without starting a renderer. The sweep (31 components convert, 8 stay) becomes **7a-1**, ahead of everything else, and turns the harness from bUnit-first into ordinary xUnit + Moq with bUnit kept for the render-dependent minority. `MLQT.Shared` also joins the coverage ratchet at the end of 7a, measuring `.razor.cs` and ignoring markup.

**7a-1 shipped the same day.** All 31 components converted, the three largest verified line-by-line against their pre-move source; Release build clean at 0 warnings and 4,453 tests green. Three things the plan did not foresee: `MLQT.Shared` needed a `GlobalUsings.cs` mirroring `_Imports.razor` (a `.razor.cs` does not read it); the csproj's `SupportedPlatform browser` was template boilerplate that had never been true, and only surfaced because moving code out of a `.razor` puts it in front of analyzers for the first time; and **`SharedUiConventionTests` was silently defeated by the sweep** — its four checks enumerate `*.razor`, so after the move they ran over markup with no code in it and all five passed. Its own can-I-see-the-source guard missed it by counting files rather than the material the checks consume. Fixed, and the guard now asserts it can see event subscriptions. That is B97's lesson recurring one level up, and it is the warning for the three new guards 7a-2 adds.

**7a-2 followed.** `MLQT.Shared.Tests` exists — the project `SharedUiConventionTests` always said it belonged in — with six code-behind policy guards rather than the three planned, and `MlqtComponentTestBase` for the render-dependent minority. Every guard was verified by breaking the source and watching it fail, which is what 7a-1's lesson demands and what changed one of them: the namespace guard can only fire against sources the build has not seen, so a sixth was added for the failure the compiler cannot see — an orphaned `.razor.cs` left behind by a renamed component, which builds cleanly and never runs. The exemption ledger is empty on purpose: all eight components that kept an `@code` block are under the limit, and an empty ledger says so where eight entries would have read as eight accepted debts. The two defects 7a-1 exposed are fixed. `MLQT.Shared` is still out of the coverage ratchet — that is **7a-5**.

**7a-3, 7a-4 and 7a-5 followed.** 7a-3 put 202 tests over `MLQT.Shared`, every one verified by mutation rather than by going green — which changed the tests three times, most sharply on the diff algorithm, where property tests that prove a script *valid* pass happily on a non-minimal one. 7a-4 took **737 lines out of `MainLayout`** (2,635 → 1,898) in ten extractions, each with tests and none needing a mock, ending with both formatting paths behind `IFormattingPipeline` — the service the 7a-6 test host registers. It also found three duplicated or dead rules along the way, including one `MainLayout` was carrying against `DirectedGraph.AddFileContainsModel`. 7a-5 brought `MLQT.Shared` into the coverage ratchet at 80% with 29 reasoned ledger entries, and **rejected the file filter the plan called for**: measured, a component's `BuildRenderTree` is not counted at all, and filtering `.razor` would have removed five classes from the report — B104's failure mode, one step after fixing it.

**Then the whole solution moved to xUnit v3**, which 7a-2 had deliberately avoided: it pinned bUnit at 1.40 because 2.x needs v3 and the other eight suites were on v2, and starting the project's first UI tests on a superseded framework was the wrong trade. Measuring it showed the source cost was nearly nothing — no `IAsyncLifetime`, two files importing `Xunit.Abstractions`, 8,669 unchanged `Assert` calls — and that the cost is the runner: on the .NET 10 SDK the VSTest target refuses to run a Microsoft.Testing.Platform project at all, and `global.json`'s opt-in covers every project in the repository, so it was nine or none. All nine moved; every suite kept its exact test count. It also turned up `ModelicaParser.Tests/TestDebug.cs`, a top-level-statements scratch file committed since the initial commit whose statements became the entry point once the project was an `Exe` — the 1,871-test suite ran nothing until it was deleted, and the `NoWarn CS7022` that had been covering for it went with it. And it exposed **B104**: a baselined class that vanishes from the coverage report is reported as debt paid.

**7a-6 built the host and 7a-7 captured the baseline.** `MLQT.TestHost` is an ASP.NET Core Blazor
Server host over the same `MLQT.Shared`, driven by Playwright through Chromium, with `AddMlqtCore()`
as the shared composition root and the host page generated from `HostAssetManifest` — the pattern the
Photino host should copy. Three host differences surfaced in the first hour, which is the whole
argument for building this before the migration rather than during it: prerendering breaks MLQT
(components issue JS interop during initial render, legal in a webview and an exception during static
render), and RCL static assets failed to serve twice for two unrelated reasons — the second because
the static-web-assets manifest is named after the *application* and the in-process entry assembly was
the test project. Two journeys also passed **vacuously** on first write and were fixed to bite: one
against a fixture that was already canonically formatted, one against a rule that ships off.
...
differently but two probe sets that drifted apart while their intersection still matched. Two probes
...
results, and writing tests for them found the same shape five times: a rule with more than one
implementation, or a rule nothing had ever asked a question of. The three VCS dialogs each carried a
byte-identical private copy of the blocking-status list, the conflict map, the relative path and the
status glyphs — and all three copies were **wrong in the same way** (B106), because a rule written out
three times is a rule nobody maintains. The glyph copies had also drifted from
`VcsStatusHelper`, which already held that table for the browser and the change list and held it more
completely, so the same file looked different in two windows of the same application. **B109 and
B110** were both in `AddRepositoryDialog` and both invisible without a renderer: a validation path
that left the dialog spinning for the session, and a failure path that closed the dialog reporting
`Ok(null)` so `MainLayout` treated it as success. **B107 and B108** were in the settings page's theme
presets; B108 is fixed and B107 is pinned and left open, because what happens to colours a user typed
in is a product decision rather than something to change while extracting a method.

The Layer 1b pattern the note called for now exists: `ShowDialogAsync` on `MlqtComponentTestBase`
...
**7b-5 and 7b-6 are complete (2026-09-08/09): the host is verified on both platforms.**
`MLQT.Photino` answers all 16 probes exactly as MAUI did under WebView2 *and* under WebKitGTK, and both
captures are committed so the comparison is a test rather than a scrollback. The manual checklist — the
part 7a says explicitly that no automation covers — has been run on each: **visual fidelity, native
window behaviour and a real file dialog are correct on both.** Three differences a user could find were
found and two are fixed (the window opened smaller on Windows, B131; the taskbar icon, B132), the third
being that **on Wayland MLQT has no icon anywhere** without an installed desktop entry (**B134**) —
which like B132 is not a defect in the process at all, so both belong to packaging. 7a's two leftovers
land here as promised: `desktop-selftest` runs the probe route on a real runner per platform, turning
the committed captures from a record into a live check, and it is **green on both** — after three runs
that found a missing shared library on Linux and a Windows leg that had been reporting success without
waiting for the host (**B142**). `nightly-webkit.yml` runs the journeys under WebKit and is the one
deliverable that **ships unproven**: `workflow_dispatch` resolves on the default branch, so it cannot
be triggered until the merge (**B143**). **What is left before cutover is 7b-7, packaging.**

**7b-7 has both installers (2026-09-10).** One per platform, each carrying the GUI, the `mlqt` CLI
and the MCP server, and each **proved by running what it installs** rather than by building cleanly:
the Windows Inno setup was installed and uninstalled end to end, and the Linux `.deb` — 155 MB of
self-contained publish into 40 MB — is extracted and asked the same three questions the staging
...
**The branch was reviewed against both phase 7 notes before merging (2026-09-11), and the plans — apart from one item — were delivered.**
The `/selftest` probe set is *stronger* than the fourteen 7a specified: sixteen probes, and the 
one the plan wrote as "each of the 14 library scripts defines its global" is two probes, because 
`klayjs` defines no global and the question worth asking about it is whether its layout 
registers. The item that was missed is **Playwright traces on failure** (B148), which the plan 
asked for and gave the reason for; it matters more now than when it was written, because the 
journeys run on two platforms and the WebKit job has never run at all.

**Five other findings came out of the review, and two are this phase's own work.** The GUI 
self-test is bounded where CI runs it and unbounded where the packaging scripts do (**B146**). 
B128's interface cache was measured for time and not for memory: 614 MB to 733 MB on MSL, and 
nobody has measured it on a graph an order of magnitude larger (**B147**). A restored window is 
not checked against the current monitor layout, so an undocked laptop gets one it cannot reach 
(**B149**). Neither installer is exercised doing an upgrade, which is what every user after the 
first release does (**B150**). And **B151** is pre-existing rather than this branch's: the graph 
builder still swallows a class's resource-analysis failure in silence, which is the shape the 
checking path had fixed out of it twice.
...
orchestration reduce no migration risk, while journeys do — the six that exist moved that number from
11.5% to 19.4%, and they run on both platforms. About 700 lines across `VCSHistory`, `BranchSelector`,
`Dependencies` and the settings tabs are at **0%** and journey-reachable. Written before the port they
are evidence about it; written after, they are just tests. `AppState` was tested outright (its ledger
entry said "nobody has written the tests", which is an admission, not a constraint) and the rest of the
debt is **B119**.
...
renderer. Then run the WebKitGTK spike, then build the Photino host and port the platform services, then **validate on Windows first**
...
| B20 | **The three largest components hold the logic phase 7a has to test** | branch review 2026-09-03 | ⭐⭐⭐ | L | **Widened by B73 (2026-09-04):** `MainLayout.razor` is **3,045 lines** — larger than either page this item originally named — and roughly 2,500 of them are C#: the whole formatting pipeline, the VCS reaction path, the deferred-analysis orchestration and the theme builders, inline in a layout component. B65 was a defect that lived in it and nowhere else, which is this item's own argument. Take it first. `CodeReview.razor` is 2,252 lines and `MetricsDashboard.razor` 978, both with analysis, VCS and persistence logic inline — against this repository's own instruction to keep business logic in services, not Razor components. It has not hurt yet because nothing tests them. Phase 7a is bUnit component tests plus a `/selftest` conformance route, and both are far harder against a component of that size: the logic can only be reached by rendering the whole page and driving the UI. This is the cheapest it will ever be to extract — before the tests are written against the current shape, and before the Photino port moves the same code again. Sequence it into 7a, not after. **Planned in detail (2026-09-07)** as phase **7a-4**, and preceded by **7a-1**, the code-behind sweep: the `@code` block moves to a `.razor.cs` partial first, unchanged, so the extraction afterwards is a C#-to-C# refactor the IDE can perform and `git diff` can show. B73's objection — that this moves the same code twice — applies to extracting twice, not to a compiler-verified cut-and-paste that makes the expensive move cheaper. Destinations are named per method in [design-phase7-gui-tests.md](design-phase7-gui-tests.md) §7a-4: `IAnalysisPipeline` for the pipeline, `MlqtTheme` for the four palette builders, the code-behind for the progress state machine and event wiring. **⚠️ shipped for `MainLayout` only (2026-09-07)** as 7a-4 — ten extractions, `MainLayout` 2,635 → 1,898 lines of C#, each extraction tested and every test verified by mutation. **`CodeReview` and `MetricsDashboard`, the other two components this item names, were never extracted**, and this row was marked shipped anyway. The coverage ledger has said so in writing since 7a-5 — "the largest page, and the one 7a-4 has not reached yet", and "the remainder of B20 for this file" — so two committed artefacts disagreed for a day. Carried forward as **B119**. It landed as `IFormattingPipeline` rather than the planned `IAnalysisPipeline`: the formatting half is work that runs identically with no UI attached, while what remains in `RunStartUpAsync` and the four `RunDeferred*Async` methods is sequencing — guard a flag, raise the progress dialog, call a service, set a step colour — which is what the note said should stay in the component, so it was left there rather than moved behind a progress seam for its own sake. The extraction also found three duplicated or dead rules, including a detach `MainLayout` performed that `DirectedGraph.AddFileContainsModel` already does. |
...
| B101 | **"Default" means two different things in the same rule reference, and one of them contradicts the prose above it** | seventeenth review 2026-09-07 | ⭐⭐ | S | `settings-reference.md` has two rule tables with a **Default** column. In the per-class table it means *enablement* and every row reads `Off`. In the Static Analysis table it means *the severity the rule takes once you switch it on*, so the rows read `Error`/`Warning`/`Info` — two lines under a sentence stating those rules are "**off by default**". A reader comparing `MLQT.Doc.ClassDescription \| Off` with `MLQT.Duplicate.Declaration \| Error` concludes the second is on and already failing their gate. Verified against the code: `StyleCheckingSettings.SeverityFor` returns `RuleSeverity.Off` for any rule not present in `RuleSeverities`, and the map starts empty, so **every** rule is off until enabled — the Static Analysis column is describing something other than a default and calling it one. **✅ shipped (2026-09-07)** — the Static Analysis column is now headed **Severity when on**, with a sentence above it saying that is the level a rule reports at once switched on and not a statement that it is running. `EveryStatedDefaultIsTheDefaultTheCodeUses` now reads each table's header and asks the question the header asks: a column headed "Default" is checked against `SeverityFor` on fresh settings, one headed "Severity when on" against the catalogue. |
| B102 | **Four documented setting names appear nowhere in the app** | seventeenth review 2026-09-07 | ⭐ | S | The **Setting** column of the per-class rule table is the name a user looks for in the Edit Repository Details dialog. Eleven of the fifteen match a real label. Four do not, and they are the section-ordering rules: the doc says **One of each section**, **Initial equation/algorithm first**, **Initial equation/algorithm last** and **Don't mix equation and algorithm sections**, while the dialog shows full sentences hardcoded in `SettingsRepositories.razor` ("A class may only have 1 public, 1 protected, 1 equation or algorithm section") or in `RuleSettingsLayout` ("A class may only have either an equation or algorithm section, not both"). There are now **three** vocabularies for these rules — the dialog label, the `RuleCatalog` title ("Initial sections first") and the doc — and no two agree. A user searching the dialog for the documented name finds nothing. **✅ shipped (2026-09-07)** — all eight affected rows now carry the dialog's exact label. Two more than the four first reported turned up once the guard's regex was widened to accept a rule-id cell with trailing text: **Imports first, extends at top** and **Components before classes** had escaped the first sweep for that reason alone. The short names the rest of the documentation uses are mapped to the exact labels in a note under the table, so the prose in `code-formatting.md`, `metrics-dashboard.md`, `cli.md` and `modelica-concepts.md` still lands — and the `#one-of-each-section-is-required-by-the-rest` anchor those pages link to is untouched. |
| B103 | **The guard on rule documentation cannot fail, and checks the wrong thing** | seventeenth review 2026-09-07 | ⭐⭐ | S | `RuleDocumentationTests` opens with `if (docs is null) return;` — run anywhere the sources are not, it passes without reading a page, which is B100's shape exactly (a gate that cannot fail) on the guard protecting every SARIF `helpUri`. B97 noticed this and made `SharedUiConventionTests` deliberately unlike it rather than fixing it. Worse, what it asserts is that the rule **id string occurs somewhere on the page** — so B101 and B102 both sit on rows that contain their id and satisfy it completely. The check to make is that each row's Setting label matches a real UI label and its Default column matches `SeverityFor` on empty settings; both are mechanical, and both are the assertions that would have caught these two. **✅ shipped (2026-09-07)** — `DocumentationDirectory()` asserts instead of returning null, so all five checks fail rather than pass where the sources are not; and two checks were added that ask about the row rather than about the id: every **Setting** cell must be a label `RuleSettingsLayout` or `SettingsRepositories.razor` actually shows, and every default/severity cell must match the code. Both were written before the fixes and watched to fail on exactly B101 and B102 — eleven rows and six labels — before either was touched. One row is still unguarded: **Composition must have components before classes** has no rule id to bind to, so its label was corrected by hand and nothing holds it there. |
| B104 | **A baselined class that vanishes from the coverage report reads as fixed** | xUnit v3 migration 2026-09-07 | ⭐⭐ | S | `check-coverage.ps1` guards the case its own header names — a *suite* that produced no report — and the case one level up, an *assembly* missing from the merged summary. It does not guard the class: a baseline entry whose class is absent from the report is reported as "now meets the bar and can be dropped", which is the same sentence it prints for a class that was genuinely fixed. Found because the collector swap made it happen: `MLQT.McpServer::Program` is top-level statements, so it is compiler-generated, and `coverlet.MTP` excludes generated code where `coverlet.collector` measured it at 0%. That particular removal is **correct** — the script header already says generated code is out of scope — but nothing distinguished it from a class that stopped being measured for a bad reason, and the only signal either way was a line saying the debt had been paid. The fix is to treat an absent baselined class as an error unless it is recorded as deliberately excluded, with a reason, the way every other entry in that ledger already is. **✅ shipped (2026-09-07)** — `recovered` now means *measured and meeting its bar*, and a baselined class the report does not contain is its own outcome: the gate fails and says so. The baseline gained an `excluded` map carrying a reason per entry under the same rule the debt entries follow, `-UpdateBaseline` moves a newly-unmeasured class into it with the `TODO` placeholder the gate then refuses, and a listed class that starts being measured again also fails — a stale exclusion would hide a later regression in it. `MLQT.McpServer::Program` is recorded there with why it cannot be measured, which restores the entry the collector swap had silently dropped. All four checks were verified by breaking the ledger and watching each fail. |
| B105 | **"Path relative to the library root" is implemented twice** | phase 7a-3 2026-09-07 | ⭐⭐ | S | `CodeReview.ReportPathOf` and `MLQT.Cli`'s `CheckReport.RelativeFileFor` compute the same thing for the same reason — the `File` field of an export, relative to the library, forward slashes — and the first one's doc comment says so in as many words, adding that it *used to* write the absolute path and was therefore "the second field the two exports were said to share that did not agree". It is the shape this backlog has named repeatedly: one rule, two implementations, and a promise that they match held by nothing. No test can hold them together where they are: `MLQT.Shared` does not reference `MLQT.Cli` and should not. The fix is to move the rule into `MLQT.Services`, which both already reference, and have each call it — after which one test covers both. Found while writing 7a-3's tests for `ReportPathOf`, which pin its behaviour but cannot pin the agreement. **✅ shipped (2026-09-07)** — and it was **two** rules, not one: `LineFor`/`FileLineOf` were duplicated the same way, so both report fields a reader acts on were written twice. `MLQT.Services/Checking/ReportLocation.cs` is now the single answer to "which file and which line does this finding point at", and both surfaces call it; agreement is structural rather than promised. Both callers' existing suites passed unchanged, which is what shows the behaviour is identical. Writing `ReportLocationTests` also found a **documentation defect carried by both copies**: each claimed a file outside the library fell back to the absolute path, and neither ever did — `Path.GetRelativePath` returns a walking-up path, which is not rooted, so it is returned as-is. The fallback only fires for paths that cannot be related at all, which on Windows means a different drive. The behaviour is left alone and the description corrected: those paths resolve against the library the report names, and changing what a report prints is not a refactor. |
| B106 | **A staged rename lets a merge or rebase start over a dirty working copy** | phase 7a-3 2026-09-07 | ⭐⭐⭐ | S | `GitMergeBranchDialog`, `MergeBranchDialog` and `GitRebaseDialog` each decided which working-copy changes block the operation, and each listed the same five statuses: `Modified`, `Added`, `Deleted`, `Untracked`, `Conflicted`. `VcsFileStatus` has **six** members. The missing one is `Renamed`, which `GitRevisionControlSystem` really does produce for a staged `git mv` — and `GetWorkingCopyChanges` returns only files with real changes, dropping `Unaltered` and `Ignored`, so every status it can report is a change. A user who had staged a rename was told the working copy was clean and offered the merge; git then refused it over uncommitted index changes, which is a far worse place to find out than the dialog that had just asked. **✅ shipped (2026-09-07)** — one rule in `MLQT.Shared/Dialogs/VcsConflictRules.cs`, all three dialogs calling it, and the statuses still written out rather than made unconditional so a member added later is a decision. `EveryStatusTheVcsCanReport_Blocks` enumerates the enum and fails on any that does not, which is the guard that would have caught this. |
| B107 | **Choosing the Light theme silently discards a custom palette** | phase 7a-3 2026-09-07 | ⭐ | S | `SettingsUI.ApplyPresetUITheme` resets all ten custom colours when "Light" is chosen, and `SettingsService.SetAsync` runs immediately afterwards, so the user's hand-built palette is gone from disk. "Dark" does not do this, and an unrecognised name falls back to Light *without* the reset — so of three paths that end in `Theme.Light`, one is destructive. Custom → Light → Custom returns the defaults; Custom → Dark → Custom returns the user's colours. Two preset buttons side by side, one destructive, with nothing on screen to say so. **✅ shipped (2026-09-08)** — no preset touches the custom palette now. Light follows the same path as Dark, and the switch is written so the asymmetry cannot return: Light, Dark and an unrecognised name differ only in which `Theme` they select, with no arm left for one of them to do something extra in. `ResetCustomPaletteToDefaults` went with it, being called by nothing once the Light arm stopped, and `NoPresetTouchesTheCustomPalette` asserts the general form over every preset name rather than the one that was wrong. It was pinned rather than fixed at first because it decides what happens to colours a user typed in, which is a product decision and not something to settle while extracting a method. The ten literal colour values were also a third copy of the `UISettings` property defaults, held to them by nothing; that half **is** fixed — the reset now copies from a default instance. |
| B108 | **Changing the UI theme hides the custom syntax colours** | phase 7a-3 2026-09-07 | ⭐⭐ | S | `SettingsUI` asked "is the syntax palette hand-edited?" two ways: on load it read the stored theme name, and after applying a preset it set the flag to `false` unconditionally. Changing the UI theme re-applies the syntax preset — deliberately, so the code background follows light/dark — so switching theme while on a custom syntax palette turned the colour pickers off while the stored name was still "Custom". The settings said Custom, the page showed presets, and reopening the page brought the pickers back, which makes it read as a rendering glitch rather than a rule. **✅ shipped (2026-09-07)** — `ThemePresets.IsCustomSyntax` is the single answer and both call sites use it. |
| B109 | **A bad path leaves the Add Repository dialog spinning for the session** | phase 7a-3 2026-09-07 | ⭐⭐ | S | `AddRepositoryDialog.AddRepository` set `_isLoading = true` and raised the overlay, then ran four validation checks that `return` early. The only place `_isLoading` was cleared was the `finally` of a `try` that began *after* those returns — so choosing a directory that does not exist put the dialog into a loading state it never left, with its own error message behind the overlay and no way out but Cancel. **✅ shipped (2026-09-07)** — validation runs before the spinner goes up, and the rule itself is `MLQT.Shared/Dialogs/AddRepositoryInput.cs` with 17 tests over the messages, which were previously reachable only by opening the dialog and getting it wrong on purpose. |
| B110 | **A failed Add Repository closes the dialog reporting success** | phase 7a-3 2026-09-07 | ⭐⭐⭐ | S | When `AddRepositoryAsync` failed, the dialog set `_errorMessage` and then fell through to `MudDialog.Close(DialogResult.Ok(_addedRepositoryId))` with the id still null. The message was computed and thrown away with the dialog that would have shown it. Worse at the other end: `MainLayout` checks `!result.Canceled`, so it treated the failure as success and switched the UI into repository mode for a repository that had not been added. The user saw the dialog vanish, no error, and no repository. **✅ shipped (2026-09-07)** — the failure path returns instead of falling through, and `AddRepository_WhenTheAddFails_StaysOpenAndSaysWhy` was watched failing against the old code. |
| B111 | **Logging was owned by a layout component, and the probe that should have caught it passed anyway** | phase 7b prep 2026-09-08 | ⭐⭐⭐ | S | `LoggingService.Initialize()` was the first line of `MainLayout.OnInitializedAsync`, so a host had logging only once that particular component rendered. The `/selftest` route runs under `EmptyLayout` by design — the whole point being that no application is booting underneath it — so **the conformance route had no logging at all**, and neither would any future host or route that does not render `MainLayout`. The `logging.writes` probe existed to notice exactly this and did not: it asked whether the log folder contained any `*.log` file, which is true on every developer machine where the app has ever run and false only on a clean one. It reported `Pass` against a host that had not initialised logging, and that false `Pass` was captured into the committed MAUI baseline. **A clean Linux CI runner was the first thing to disagree** — which is the argument for the Linux job stated as an event rather than a principle. **✅ shipped (2026-09-08)** — `LoggingService.Initialize()` moved to `AddMlqtCore()`, the composition root every host calls, beside the invariant-culture setup that is there for the same reason; the probe now writes a unique token and asserts *this run's* line reached the file, reading the directory NLog was actually configured with rather than recomputing it; and the baseline was re-captured so its pass is a real one. Verified by putting the defect back and watching the probe fail **on Windows**, where the old one passed. |
| B112 | **The coverage gate could not run, and had not run for three commits** | phase 7b prep 2026-09-08 | ⭐⭐ | S | `build/check-coverage.ps1` names the suites it measures and runs each with `--no-build`; the `code-coverage` job builds them beforehand. Two lists, one rule, nothing holding them together. 7a-5 added `MLQT.Shared.Tests` to the gate and not to the workflow, so `dotnet test --no-build` had no assembly to run and the job was guaranteed to fail with "MLQT.Shared.Tests did not pass; coverage from a failed run means nothing" — the script's per-suite exit-code check, which fires before the report count it was first assumed would catch this. **Nobody found out for three commits**, because `code-coverage` `needs: build-libraries` and the jobs it depends on kept failing first, so it was skipped every time. A gate that never runs is not a gate, and it is worse than none: the branch looked as though it had coverage enforcement. **✅ shipped (2026-09-08)** — the job builds the missing suite, and `CoverageSuitesTests` holds the two lists together in **both** directions: a suite measured but not built fails the run, and a suite built but not measured is wasted time and a misleading job name. Verified by removing the workflow line again and watching the guard name the exact suite. |
| B113 | **Each platform tested what the other did not** | phase 7b prep 2026-09-08 | ⭐⭐⭐ | S | All nine suites ran on `windows-latest` and nowhere else; the browser journeys ran on `ubuntu-latest` and nowhere else. So the only end-to-end coverage of the shared UI never ran on the platform the product ships on, and the six suites most likely to behave differently on Linux — `RevisionControl` spawns git and svn processes, `MLQT.Services` does the encoding and path work `ModelicaFileEncoding` exists for, `ModelicaParser` carries a 95% bar because it is critical — had never run there at all. For a phase whose entire deliverable is a Linux UI, that is the wrong way round. **It was not hypothetical**: the first Linux job this repository ever had (`cli-linux`, on main) found six `MLQT.Cli` tests built around a literal Windows drive path within a day of being added, and its own commit message says "The Linux job found them, which is what it is for". That job also never reached this branch, which forked before it landed — so the branch had *less* Linux coverage than main while being the branch about Linux. **✅ shipped (2026-09-08)** — a `linux-tests` job mirrors the Windows suites exactly, with the same SVN exclusion so the two are comparable, and folds in `cli-linux`'s publish-and-smoke-test of the shipped `linux-x64` binary so the merge to main loses nothing; the journeys become a two-leg matrix over Linux and Windows. `WorkflowPlatformParityTests` holds the two jobs together in both directions and asserts neither portable job installs the MAUI workload — verified by dropping a suite and a matrix leg and watching it name each. |
| B114 | **Eighteen tests assumed Windows paths; the code under test did not** | phase 7b prep 2026-09-08 | ⭐⭐ | S | The first full Linux run failed 18 tests across `ModelicaGraph.Tests` and `MLQT.Services.Tests`, all on path strings. **In every case the product was already correct** — `ResourceTreeHelper.FindCommonDirectoryRoot` splits on both `Path.DirectorySeparatorChar` and `AltDirectorySeparatorChar` and joins with the platform's own; `FileMonitoringServiceHelpers.IsInHiddenDirectory` does the same; `FileNode.FileName` is `Path.GetFileName`. Only the fixtures assumed Windows, and a literal like `C:` + backslash + `Projects` is an *absolute* path on Windows and an ordinary **relative** one on Linux. Worse than the failures were the passes: four `FormattableModelicaFiles` tests assert `Empty` and passed on Linux because nothing matched at all, so they would have passed against a completely broken rule. **✅ shipped (2026-09-08)** — `TestSupport/TestPaths.cs`, linked into both test projects so the rule has one implementation, and 63 literals converted. One test could not use it: nothing-in-common needs two volumes on Windows and two differing first segments on Linux, so it branches, and says why. |
| B115 | **A heredoc turned a test literal into a two-line string, and the test passed anyway** | phase 7b prep 2026-09-08 | ⭐ | S | `VcsChangeResolverTests` carried `[InlineData(@"Lib` + backslash + `readme.md")]`. Writing the file through a shell heredoc collapsed the backslash-r into a carriage return, leaving a *verbatim* string spanning two lines — which compiles, because `@"..."` may contain newlines. The test asserts `Empty` on a non-Modelica file, and a filename containing a newline is not a `.mo` file, so it passed while testing nothing. Introduced by `e9eb24c` (7a-4) and unnoticed for a week. **✅ shipped (2026-09-08)** — the theory now uses forward slashes normalised in the body, which an attribute argument can hold without escapes at all. **The general lesson is about the tooling, not the test**: a shell heredoc is not a safe way to write source containing backslashes, and the failure is silent in exactly the cases where the result still compiles. |
| B116 | **An OpenModelica test fails against a real omc and was being excused as a missing tool** | phase 7b prep 2026-09-08 | ⭐⭐ | S | `SimulationTests.GetErrorStringAsync_AfterClear_ReturnsEmpty` fails at its *arrange* step: after `LoadFileAsync("NonExistent.mo")`, `GetErrorStringAsync` returns empty, so `Assert.NotEmpty(errorBefore)` fails before the behaviour under test is reached. OpenModelica **is** installed (1.26.0, at the absolute path `OpenModelicaFixture` hardcodes), so this is a real disagreement between the test's assumption and what omc 1.26 reports — not a missing tool. It was hidden twice: the suite runs in no CI job, and `run-all-tests.ps1` initially excused failures in tool-dependent suites by category, which is how a script comes to excuse the one real failure it found. **✅ shipped (2026-09-08)** — **the test was always wrong, and wrong twice.** Probing omc 1.26 directly settled it: `loadFile` on a *missing* file answers false and reports **no** error text (it separates "could not open it" from "opened it and it was wrong"), so the arrange step could never produce the error the test needed. And `clear()` does **not** discard pending errors, which is what the test claimed to prove — it passed historically only because `getErrorString()` *drains* the buffer, so the post-clear read was empty for a reason that had nothing to do with `clear()`. Replaced by three tests that assert what omc actually does: a missing file reports nothing, reading the error string consumes it, and `clear()` leaves a pending error in place. Writing them exposed a further fragility — the suite shares one omc, so its error buffer is global state and a test asserting on it must drain first, which is why the new test passed alone and failed in the suite. `OpenModelicaFixture` also hardcodes a version-specific Windows path, which will not survive an omc upgrade and cannot work on Linux at all. |
| B117 | **The coverage headline counted every NuGet package the tests touched** | phase 7b prep 2026-09-08 | ⭐⭐ | S | `check-coverage.ps1` passed reportgenerator a **deny** list — `-DymolaInterface;-OpenModelicaInterface` — so every assembly not named was measured, including every dependency the tests happened to execute. SharpCompress alone contributed **69,876 uncovered lines**; with LibGit2Sharp, Moq, FluentValidation, bunit and RecyclableMemoryStream that was **79% of the denominator**. The reported figure was **19.2%** while the code we own stood at **69.4%**. The per-class gate was never affected — it only ever looked at assemblies in `$bars` — but the headline is what anyone reads, and one that low cannot distinguish a real regression from the arrival of another transitive dependency. **✅ shipped (2026-09-08)** — the filter is now an **allow** list built from `$bars`, so the assemblies measured and the assemblies gated are one list and a new project of ours cannot be silently left out. 19.2% → 69.4%, with the same 184 classes gated and the same 40 below their bar, which is what shows the gate itself was untouched. |
| B118 | **On Linux, two absolute paths were reported as sharing nothing** | phase 7b prep 2026-09-08 | ⭐⭐⭐ | S | `ResourceTreeHelper.FindCommonDirectoryRoot` special-cases a bare Windows drive segment (`C:` → `C:` + separator) and had no equivalent for POSIX. A POSIX absolute path splits with an empty leading segment, so two of them always share that one — and joining a single empty segment gives `""`, which every caller reads as *nothing in common*. The external resources tree would therefore render **empty** on Linux for any project whose resources sit under different top-level directories, which there is the ordinary case rather than an odd one. The test file's own comment says `""` is "the scenario that was causing the empty tree". **A genuine product defect, found by running the suites on Linux for the first time** — the other 17 failures in that run were fixtures (B114), this one was not. **✅ shipped (2026-09-08)**. Fixing it also exposed two tests that contradicted each other on Linux: nothing-in-common cannot be expressed with two absolute paths there, and now uses relative ones. |
| B119 | **The half of B20 that was never done: `CodeReview` and `MetricsDashboard`** | phase 7b prep 2026-09-08 | ⭐⭐ | M | B20 named three components and 7a-4 extracted one. `CodeReview` is 1,065 coverable lines at 14% and `MetricsDashboard` 360 at 4%, and in both cases what is untested is what B20 asked to be moved: for `CodeReview`, the suppression writing, the dictionary and spelling actions and the findings export; for `MetricsDashboard`, the snapshot persistence, storage grouping and trend building. Their finding and scope decisions *are* tested — that part of 7a-3 happened. Not a prerequisite for 7b: a host swap does not change this code, and 7b touches neither file, so the argument that made B20 urgent before 7a — extract before the tests are written against the current shape — did not apply again here. **✅ shipped (2026-09-11).** Smaller than this entry reads, because two of its assumptions had expired: the code-behind extraction was already done (7a-1), and `MetricsSnapshot.AggregateByTimestamp` — the core of trend building — was already extracted and tested. What was left was exactly what the ledger entries named, and it is now in `MLQT.Services` with 50 tests: **`FindingExport`** (the export's payload, ordering and file naming), **`SuppressionScope`** (how far a rule waiver reaches and which findings it clears), **`SpellingAcceptance`** (which word is recorded and what it covers, for both the repository-wide and the class-scoped waiver), and **`MetricsStorage`** (which history file a snapshot belongs in, which repository owns a scope, and which files the trend reads). Between them the two pages lost 183 lines. **Extraction by moving, not by rewriting, and it mattered twice.** `FindingExport` was first written to compute the file line and relative path itself; both already had one implementation in `ReportLocation`, shared with the CLI, and a second copy is precisely the defect B1 left behind the first time. And `SpellingAcceptance.WordOf` was first written to read the finding's `Discriminator`, which the page explicitly does not do and says why: for a documentation finding it carries the section as well as the word ("documentation info:tyre"), so it underlines nothing. **What is still only reachable by rendering** is the orchestration around these — file picker, snackbar, re-render, scroll restoration — which is what the ledger entries for both classes now say, rather than naming work that has been done. |
| B120 | **The test host asks for an `app.css` it does not have** | phase 7b-A 2026-09-08 | ⭐ | S | `HostAssetManifest` lists `app.css` — each host's own stylesheet, as distinct from `_content/MLQT.Shared/app.css` — and `MLQT.TestHost` has no `wwwroot` at all, so every page load 404s on it. Harmless there (the test host has no styling of its own to lose) and invisible, because `OpenAndBrowseJourney` only inspects URLs containing `_content/`. Found by dumping a panel's text while chasing a journey selector, in the server log rather than by any test. **✅ shipped (2026-09-11)** — the worry about 7b-2 did not materialise: `MLQT.Photino` carries its own placeholder `app.css`, so only the test host was 404ing. It now has the same placeholder, and each file says why an empty stylesheet exists. **The journey's check is widened from `_content/` to every same-origin request**, which is the half that mattered — verified in both directions by removing the file again and watching `EveryAssetTheHostPageAsksFor_Loads` name it. The narrow filter is what hid this for a phase, in the one place a *real* missing asset would have shown up. |
| B121 | **Photino logs every render batch to stdout, and the UI feels it** | phase 7b-2 2026-09-08 | ⭐⭐ | S | Reported from real use of the first `MLQT.Photino` build: the UI is noticeably less responsive than MAUI's, **particularly while something is running in the background**. `PhotinoWindow.LogVerbosity` defaults to verbose, and a Blazor render batch is one of the messages it logs — base64-encoded, tens of kilobytes for an ordinary update. Measured on an ordinary startup: **1,084,704 bytes written to stdout in 15 seconds, against 6,026 with it off** — about 180×, and every console write is synchronous, so the cost lands on the thread producing the update and scales with how often the UI changes. That is exactly the reported shape: an idle window is fine, one with the analysis pipeline behind it is not. **✅ shipped (2026-09-08)** — silent by default in both Photino hosts, `MLQT_PHOTINO_LOG` raises it for debugging the host itself. **The mechanism is measured; the improvement in perceived responsiveness is not** — that needs the person who reported it. If it is still slow, the next candidates are Photino's message bridge against WebView2's and how often MLQT calls `InvokeAsync(StateHasChanged)` during analysis, and neither has been looked at. |
| B122 | **MAUI's `Preferences` store was looked for in the packaged-app locations, and it is an unpackaged app** | phase 7b-3 2026-09-08 | ⭐⭐ | S | Recorded first as "in none of the places it is supposed to be": not under `HKCU\Software`, not in a WinRT `settings.dat` container, not in `%LocalAppData%\MLQT`, and a registry search for the `ApplicationId` and for a known key name found nothing. That conclusion was **wrong, and it changed a design** — it forced a migration in which the MAUI app seeded the shared store from its own `Preferences` API, which requires a MAUI release that every user installs and runs *before* the Photino one. Unshippable, and only noticed when that constraint was stated out loud. The store is a plain JSON file at `%LocalAppData%\<publisher>\com.mlqtproject.MLQT\Settings\preferences.dat` — unpackaged MAUI does not use the packaged-app locations at all, and the publisher segment reads `User Name` because that is the template placeholder still in the appxmanifest. **Fixed:** `MauiPreferencesFile` reads it directly and the MAUI app is untouched. The lesson is not about `Preferences`: a search that finds nothing is evidence about the search, and "the answer does not matter" was the sentence that should have been suspicious. |
| B123 | **The coverage merge under-reports classes that several suites load and one exercises** | phase 7b-3 2026-09-08 | ⭐⭐ | M | `JsonSettingsService` is covered 104/108 by `MLQT.Services.Tests`, and the gate reports **56.5%**. Three other suites (`MLQT.Cli.Tests`, `MLQT.McpServer.Tests`, `MLQT.Shared.Tests`) load `MLQT.Services.dll` without exercising the class and report it 0/108, and reportgenerator's merge degrades rather than unions — the merged class also shows 92 coverable lines against the 108 each report holds, so the reports are not being lined up. **Four classes measurably affected**: `JsonSettingsService` 56.5% against 96.3%, `FormattingPipeline` 25.0% against 81.8%, `DymolaCheckingService` and `OpenModelicaCheckingService` 33.6% against 75.7%. Three of those already carry ledger entries blaming the code. Found while adding tests that took a class from 0% to 96% and watching the gate report 53%. **✅ closed (2026-09-11), and the diagnosis in this entry was wrong.** **reportgenerator's merge is a correct union**, checked rather than argued: over the whole merged report, **360 of 361 gated classes match a hand-computed union of the seven cobertura files exactly** (the one exception is off by three coverable lines on a partial class, not by coverage). Two separate mistakes made it look otherwise. The first was arithmetic: coverlet writes each line twice, once under the class and once under the method that owns it, so counting raw `<line>` elements doubles a class — the “108 lines” in this entry is 56 real ones. The second was worse, because it made the *low* numbers look wrong when they are right: `DymolaCheckingService` and `FormattingPipeline` really are at 33.6% and 25%, because **an `async` method's body lives in a compiler-generated state machine**, which cobertura reports as a nested class and reportgenerator folds back into its owner. The “truth” of 75.7% and 81.8% came from reading the outer class alone, which omits the body of every async method in it. The ledger entries blaming the code were right and are unchanged; `JsonSettingsService` is not in the ledger at all and reads 96.4%. **What was real is how that measurement could happen, and it is fixed.** reportgenerator unions line *numbers*, so merging a report taken before a file changed with one taken after invents coverable lines that no longer exist and that nothing ever hit — which is exactly a class at 96% reading as 56%, with a coverable count matching no version of the file. `check-coverage.ps1` cleared its results directory only when it ran the suites itself, so `-SkipTests` and any leftover run merged straight into the answer, and coverlet's `coverage.cobertura.<timestamp>.xml` naming means a directory that is not cleared accumulates rather than overwrites. **The gate now refuses both**: more than one report per suite, and with `-SkipTests` any report older than the assembly it measures. Both were verified by arranging them; the second fired immediately on real data, because the reports on disk were from the previous evening and the build was from that morning. |
| B124 | **`MLQT.McpTester` still fetches Roboto from the network at launch** | phase 7b-4 2026-09-08 | ⭐ | S | The same defect 7b-4 fixed everywhere else: `MLQT.McpTester/wwwroot/index.html` links `fonts.googleapis.com`, so a manual diagnostic tool needs a network to start looking right. Left alone rather than fixed because McpTester has **no project reference to `MLQT.Shared`**, so it cannot use `_content/MLQT.Shared/fonts` and the fix is either a second 231 KB copy in its own `wwwroot` or a reference added for a stylesheet. Neither is obviously right for a tool nobody ships. **✅ shipped (2026-09-11)** — neither was chosen: it asks for the **platform UI font** instead, through a `MudTheme` whose `Typography.Default.FontFamily` is a system stack. No download and no second copy of a webfont, at the cost of McpTester looking slightly unlike MLQT — the right trade for a diagnostic tool. Measured rather than assumed: of MudBlazor 9.6's fourteen typography variants only `Default` carries a font family out of the box and every other falls back to it, so one setting is the whole change. The guard is deliberately broader than the defect — `NoHostPageInTheRepositoryFetchesAnythingFromTheNetwork` enumerates **every** `wwwroot/index.html` in the repository, because 7b-4 enforced this over the *manifest* and McpTester is precisely the host that does not use one. **Its first version passed against the unfixed page**: a shell heredoc had turned the pattern's word-boundary escape into a literal backspace, so it matched nothing, which is why it now asserts that the scan found assets at all. |
| B125 | **Photino felt slower than MAUI; a same-day A/B says it is not** | phase 7b-5 2026-09-08 | ⭐⭐⭐ | M | Three slow things were reported. The application log had already recorded all three, on the same library, across both hosts, for weeks — **it was never read**, and none of the three was the host. Settled by opening the same project in each build one after the other: same repositories, same 39,860-model dependency analysis, same `Claytex` check. **Photino 426 s against MAUI 2026.4.0's 434 s** (dependency analysis 75 vs 78 s, style check 297 vs 299 s) — and Photino did it carrying **97 encrypted reference libraries to MAUI's 53**, a 118,612-model graph against 77,860. Faster on half again the workload. The cross-day figures that suggested a 30% regression (MAUI 324/325 s on 7 Sep, Photino 420 s on 8 Sep) were variance: MAUI itself ranged 183–468 s on 2 Sep. **Closed.** The two that were real are B126 and B127; B128 is the growth the series exposed. |
| B126 | **Removing a node from the graph is O(n), and every caller removed nodes in a loop** | phase 7b-5 2026-09-08 | ⭐⭐⭐ | S | `DirectedGraph.RemoveNode` walks every node's edge set, because nothing indexes edges by target. Five call sites removed a file's classes in a loop over it, so removing *m* nodes from a graph of *n* cost *m × n*. On an ordinary file *m* is one and nobody notices. On a generated FMU interface holding **4,478 classes** in a 39,860-node graph it is ~178 million set operations: correcting one spelling mistake took **54 seconds** with the window doing nothing (log, 16:53:30 → 16:54:24 on 2026-09-08). `LibraryDataService.RemoveModelsFromFile` had a second one beside it, walking every library's `ChildrenByParent` once per removed model. **Fixed:** `RemoveNodes` makes one pass, the index cleanup is set-based; measured at those sizes, 5,746 ms → 22 ms and 210 ms → 3 ms. Found while looking for a Photino regression; it is not one, it was always quadratic and only a file this size makes it visible. |
| B127 | **Adding a repository ran its analysis behind a snackbar rather than the progress dialog** | phase 7b-5 2026-09-08 | ⭐⭐ | S | Every other path that runs dependency analysis, external resources and style checking does it behind the modal progress dialog, which is what stops a user starting something else in the middle of heavy background work. `OpenAddRepositoryDialog` announced itself with a snackbar and ran free, so the UI crawled and stayed clickable — worst of both. Reported from real use and **explicitly not a migration regression**: it has been that way since it was written, and only became noticeable because the repository already had rules in its `.mlqt/settings.json` so the check started immediately. **Fixed**, waiting on `WaitForCompletionAsync` rather than on the completion event, because that event fires from the service's thread and racing it leaves a modal dialog nobody can close. |
| B128 | **The style check on one library has roughly tripled since late August** | phase 7b-5 2026-09-08 | ⭐⭐ | M | The series that came out of B125's log analysis: **~105 s** median over 24–27 Aug, ~190 s on 1–2 Sep, **~300 s** now, on the same repository. **Expected growth rather than a regression**, and confirmed as such by the owner: the intervening weeks added the phase-5 suppression reads and the phase-6 whole-graph analyses and coverage measurement, more rules are enabled, and the project now loads far more libraries including the encrypted ones. Recorded because the figure is worth knowing and worth reducing eventually. What would make it tractable is a breakdown per rule and per analysis, which nothing currently reports. **✅ shipped (2026-09-11): the breakdown exists, and it found something.** `CheckTimings` records every phase of a run — the parse, each style rule by name, each whole-graph analysis, the per-finding tail — for two timestamp reads apiece, so it is always on; `mlqt check --timings` prints it and the desktop app writes it to the log, which is where a performance question is investigated from here. **What it found, measured over the Modelica Standard Library (6,487 classes):** the per-class rules were 86% of the work, and within them `SpellCheckDescriptions` alone was 50% — against `SpellCheckDocumentation`, doing the same kind of work over the same text, at 2%. That asymmetry was the clue: **the rule was not slow, it was being billed for shared lazy work.** Resolving a class's inherited element names happens once per class and is cached, so whichever spell rule reaches a class first pays for all of it. Timed where it is spent, it was **49% of the whole check** — 122 thread-seconds over 5,631 classes — and all of it was re-extracting the interface of base classes, once for every class that inherits them, each extraction re-parsing a class whose tree the checker had deliberately released. **Fixed by caching the extracted interface for the length of a run** (`ClassElementResolver.InterfaceCache`, passed in rather than static, because a class's source changes under the desktop app while it is open). Inherited names: **122s to 6s**. The same cache in `ShadowingAnalyzer`, which walks the same chains, took it out of the top ten entirely. Total measured work **248s to 122s**, wall clock on this machine **31s to 21s**, and the findings are **byte-identical** before and after — 17,784 of them, compared as JSON. **A second, smaller one on the way past:** `SpellChecker.IsCorrect` took a lock around the accepted-word list on every call, which is once per word of every description in a library. A micro-benchmark scaled 4.8x on 24 threads instead of ~24x. The list is now an immutable snapshot swapped on write and the dictionary answer is remembered per word: 3.2us to 0.11us per call. It barely moved the total — which is the point of measuring rather than guessing, because it was the first thing that looked wrong and the second that mattered. **Left as it is:** `parse` at 21% of what remains is the floor for a tool that reads source, and the three rules above it (`OneOfEachSection`, `PublicParametersAndConstantsHaveDescription`, `FollowNamingConvention`, ~20s each) are each one tree walk per class doing their own work. There is no next obvious win, and the breakdown is now there to find one if the numbers move again. |
| B129 | **A reference library reached through two settings is loaded, parsed and indexed twice** | phase 7b-5 2026-09-08 | ⭐⭐⭐ | S | MLQT has two ways to say "load this for reference" — a repository marked reference-only, and a path under reference libraries — and nothing checked whether they name the same folder. A real configuration had **all three** of its reference paths already registered as reference-only repositories, so the log shows **159 loads of 97 distinct libraries**: the standard library, `ExternData` and sixty others loaded twice. **Invisible in the graph**, which is why it went unnoticed: `AddNode` keys on the class id, so duplicate classes collapse to one node. Visible everywhere counted per *library* — two entries in the browser tree, and a model total of **118,612 against a true 77,860**, which is the number the deferred-analysis threshold is compared against — and the parse is paid twice either way. The one guard that existed covered only an encrypted library whose name matched one loaded from source, and was written inline in the loading loop where nothing could test it. **Fixed:** `ReferenceLibraryRules.ReasonToSkip` decides it, on the directory first and the name second, keeping the rule that readable source beats a library reconstructed from vendor documentation. Reported by the owner from the A/B in B125, which is where the mismatched totals showed up. |
| B130 | **The model total double-counted, by a different amount each run** | phase 7b-5 2026-09-08 | ⭐⭐ | S | Spotted by the owner when B129's fix left the total at **76,129** where MAUI had reported **77,860** — the two were expected to match once the duplicate loading stopped. They never could. `Libraries.Sum(l =&gt; l.ModelIds.Count)` counts a class once per library that lists it, and the same library is routinely loaded twice: a tool's folder ships the encrypted build of a library the user also has checked out. `ExternalStubBuilder` records a documented class **only when its source is not already in the graph**, so which library ends up listing the id depends on which parallel load finished first — the encrypted `Claytex` contributed 2,343, 2,290 and 1,637 classes on three consecutive runs of the same project across both hosts. `LoadedLibrary` entries sharing ids was already known and handled where it showed (`Owns`, and the tree keying by model id); the raw sum was the one place left. **Fixed:** `ILibraryDataService.TotalModelCount` counts distinct ids that are actually in the graph. It is the number the deferred-analysis threshold is compared against, so the old one could silently change whether an analysis was deferred. |
| B131 | **Photino sizes windows in pixels, MAUI sized them in units, so the new host opened smaller** | phase 7b-5 2026-09-08 | ⭐⭐ | S | One of the two differences a user could find between the two builds. The host asked for `1400 x 950` against MAUI's `1200 x 900` and produced a **smaller** window: on a 125% display 1400 physical pixels is 1120 units and 950 is 760, so it opened a fifth shorter from code that appears to ask for more. Which unit Photino uses was settled by measurement — with `SetSize(1400, 950)` the self-test's own `interop.dimensions` probe reported a 1106-pixel viewport, which is 1400 ÷ 1.25 less the chrome. **Fixed:** `WindowGeometry.Centred` takes MAUI's numbers, scales by the monitor, clamps to the work area and centres. **And a second defect underneath it:** `PhotinoWindow.MainMonitor` throws until after `Run()`, so reading it where the placement was decided fell into a `catch` that logs and returns a fallback — the first-run size would have been wrong on every machine forever, visible only in a log nobody reads. The scaled size is applied from a `WindowCreated` handler; verified on a fresh profile at 1500x1125, which is 1200 x 900 units. |
| B132 | **The taskbar showed the generic icon, and nothing in MLQT was wrong** | phase 7b-5 2026-09-09 | ⭐⭐ | S | Explorer, Alt-Tab and the title bar were correct; only the taskbar button was not, for every copy of `MLQT.Photino.exe` from any folder. **A stale Start Menu shortcut.** `MLQT.lnk` pointed at a publish holding a build from *before* the icon existed — an executable with no icon resources — and Windows 11 resolves a running window to its matching shortcut and takes the button's icon from there. The shortcut said "use the target's icon"; the target had none. Confirmed by moving the shortcut aside and running the same build unchanged: the icon appeared. **Fixed** by publishing a current build over the folder it points at. Two lessons worth more than the defect: **a screenshot is evidence and reasoning is not** — six code-level hypotheses were tried and disproved before anyone looked at the taskbar, and the answer was outside the process entirely; and **UI Automation names the button**, so the exact rectangle can be cropped instead of squinting at a strip of pixels on a busy desktop. The `AppUserModelID` added during the hunt is removed — it did nothing, and an id matching no shortcut is a documented way to confuse the resolver. `WindowIcon.Apply` stays, on its own merit: it gives the window 40x40 rather than a stretched 32x32 at 125%. **7b-7 owns creating the shortcut deliberately**, at the installed location, because Windows recreates it aimed at whatever it last saw run. |
| B133 | **The Photino host ran only from a publish; a built one opened a window that loaded nothing** | phase 7b-5 2026-09-09 | ⭐⭐⭐ | M | `dotnet publish` writes a real `wwwroot`; `dotnet build` writes `&lt;name&gt;.staticwebassets.runtime.json` and leaves the files where they are — in the source tree, other projects, the NuGet cache, `obj` for generated ones. The host held a `PhysicalFileProvider` over `wwwroot`, so **F5 and `dotnet run` opened a window that loaded nothing, with no error** — a webview showing nothing is indistinguishable from one still starting. 7b-2 recorded the developer story as unresolved and it stayed that way until someone tried to run a Debug build. **Fixed:** `StaticWebAssetManifest` resolves a request path through the SDK's manifest — assets first, then the nearest enclosing pattern, which is the order that matters because the project's own `wwwroot` is a `**` pattern at the tree root and consulting patterns first shadows every package's assets with files that do not exist. The published folder still wins when it is there. Verified by running the Debug build directly: 16/16 probes. **And a second one found while fixing it:** the "there are no web assets" message was logged before `AddMlqtCore` initialises logging, so the loud failure added to replace the silent one was itself silent. |
| B134 | **On Wayland MLQT has no icon anywhere, and `SetIconFile` cannot give it one** | ✅ fixed in phase 7b-7 2026-09-10 | ⭐⭐⭐ | S | The Windows taskbar finding (B132) on a different mechanism, found by the Linux manual checklist — and **recorded wrongly the first time**, which is the more useful half. It went in as "the dock is generic, the title bar and Alt-Tab are right", because the checklist question named the three places in one breath and the answer was taken at face value instead of being traced; that claim reached this backlog, the phase note and `Branding/README.md` before the owner restarted the app and saw Alt-Tab was generic too. **What the wire says:** Photino's Linux library imports `gtk_window_set_icon_from_file` (`nm -D --undefined-only Photino.Native.so`), an X11-era GTK call, and Wayland has no protocol for a client to give its own window an icon bar the recent `xdg-toplevel-icon-v1`, which this compositor does not advertise. A full `WAYLAND_DEBUG=1` trace shows the toplevel receiving exactly five requests — `set_title`, `set_app_id`, `set_parent`, `set_min_size`, `set_max_size` — and **not one of them is an icon**. So `SetIconFile` is a **no-op on a Wayland session**: the dock is not ignoring a window icon, there is no window icon. GNOME matches `app_id` to an installed **desktop entry** and takes the icon from there for the dock, Alt-Tab and the window list alike. `app_id` is `MLQT.Photino` (GTK's `g_get_prgname()`), unchanged by `dotnet run` — checked, because a launcher resolving to `dotnet` would have been the obvious thing to blame. Installing `~/.local/share/applications/MLQT.Photino.desktop` with a matching `Icon=` made the icon appear and removing it put it back, so the fix is demonstrated in both directions. **7b-7 owns installing a real one, and this is bigger than it first read**: until packaging ships, MLQT on a Wayland desktop is unbranded everywhere, not merely in the dock. `Program.ApplicationIcon`'s GTK branch stays — X11 does honour it — but it is dead code on the default Ubuntu session and must not be read as the Linux icon mechanism. The lesson is now stated twice from two operating systems: **a desktop shell identifies an application by an installed entry, not by the window** — and once more about method, that a checklist answer covering three things at once is one observation, not three. **Fixed by the `.deb` (7b-7):** `MLQT.Photino.desktop` named after the `app_id`, `Icon=mlqt` in `hicolor` at six sizes, `StartupWMClass` for X11. Packaging turned up a **third instance of the same lesson**: `/usr/bin/mlqt-gui` cannot be a symlink, because `app_id` is `g_get_prgname()` — the base name of `argv[0]` — so a symlink of that name makes the window report `app_id "mlqt-gui"` and match nothing. Measured with `WAYLAND_DEBUG=1` both ways; the launcher is a wrapper using `exec -a MLQT.Photino`, which is safe because .NET resolves its base directory from `/proc/self/exe` (16/16 probes through it) and which is a **bashism**, so the shebang is load-bearing too. `DebianPackageTests` holds the whole chain. **Not verified by machine, and it cannot be here:** that GNOME draws the icon needs the package installed and a human looking — the desktop stack was checked as far as root-less access allows (`Gio.DesktopAppInfo` resolves the entry, `Icon=mlqt` looks up to a real file at every size). |
| B135 | **Playwright ships no browser for Ubuntu 26.04, so the journeys need an override to run there at all** | phase 7b-6 2026-09-09 | ⭐⭐ | S | `playwright install chromium` refuses outright — *"Playwright does not support chromium on ubuntu26.04-x64"* — and the newest platform 1.56 knows is `ubuntu24.04-x64`. So on the main development machine **`run-all-tests.ps1`'s journey leg cannot run**, which is easy to read as a broken suite rather than a missing browser. With `PLAYWRIGHT_HOST_PLATFORM_OVERRIDE=ubuntu24.04-x64` the fallback builds download and **all 51 journeys pass under Chromium**, so this is a packaging gap in Playwright and nothing about MLQT. **WebKit is the half that does not recover**: its ubuntu24.04 build links `libicu74` and `libvpx9`, and 26.04 ships `libicu78` and no `libvpx9` at all, so it will not launch even with the override — which is why the WebKit rehearsal is a CI job on `ubuntu-latest` and not something a developer runs locally. **✅ shipped (2026-09-11)** — `run-all-tests.ps1` sets the override when `/etc/os-release` names an Ubuntu newer than the newest Playwright ships for, and says so, including that WebKit will not recover. Bumping one variable retires the workaround when Playwright adds 26.04. The decision is a function taking the *contents* of `/etc/os-release` rather than reading it, so it can be exercised for a distribution the machine is not — **which is how the first version was caught being wrong**: it built the platform name from a `[version]`, and `[version]'24.04'` renders as `24.4`, so it would have asked for `ubuntu24.4-x64` and Playwright would have rejected it. The workaround would have done nothing while looking correct. Seven cases pass now (26.04, 25.10, 24.04, 22.04, an unquoted VERSION_ID, Debian, and no os-release at all), and `TestRunnerScriptTests` holds the string-not-version half of it. Noted alongside: **`pwsh` is not installed on this machine**, so neither `run-all-tests.ps1` nor `check-coverage.ps1` runs here without it. |
| B143 | **`nightly-webkit.yml` cannot be run at all until the branch merges** | phase 7b-6 2026-09-09 | ⭐ | S | `workflow_dispatch` resolves a workflow on the **default branch**, so `gh workflow run nightly-webkit.yml --ref gui-linux-migration` answers *"not found on the default branch"* and the Actions UI offers no Run button either. Combined with **B135** — Playwright ships no WebKit build that runs on this development machine — there is **no way to exercise this workflow before it is merged**: not from CI, not from the CLI, not locally. So it is the one 7b-6 deliverable that ships **unproven**, and the argument for it is still sound (it is the only automated thing that runs MLQT's real UI on a WebKit core at all, and every defect found in 7b-6's manual pass was found by a person). **Open**, and the action is specific: run it once by hand immediately after the merge rather than letting 02:00 be its first outing, because the first run of a job written blind has failed twice already today (B142). |
| B136 | **Every log line went to the console too, and no Windows user could ever have seen it** | phase 7b-6 2026-09-09 | ⭐⭐ | S | `LoggingService.Initialize` added a `ConsoleTarget` at Info alongside the file target, commented "Console target for debugging". It was invisible for the life of the project because the host was a Windows-only `WinExe` with no console attached — so the first time anybody started MLQT from a terminal, which is to say the first Linux run, it printed its entire startup sequence and every stage of the analysis pipeline into that terminal. Reported by the owner within minutes of running `dotnet run`. **Not merely noise.** It is the B121 shape exactly: every console write is synchronous and lands on the thread producing it, and MLQT logs heavily throughout the pipeline — a second synchronous stdout writer, of the kind we had already removed from Photino's own message logging, left in place and unmeasured. **Fixed:** off by default, `MLQT_LOG_CONSOLE` turns it back on for diagnosis (a level name chooses how much; anything else that is not a level means Info, because someone setting the variable is trying to leave silence and a typo must not return them to it; `Off` is a real NLog level and is honoured). `ConsoleLevel` is lifted out of `Initialize` so the decision can be tested at all — `Initialize` configures NLog globally and runs once per process — and it is now the only thing in that class with tests that assert rather than DoesNotThrow. **The consequence is named rather than hidden**: the "there are no web assets" error that explains a blank window now reaches the log file and not the terminal, which is the trade the owner asked for — one place to look. `Documentation/troubleshooting.md` gains the section that should have existed before it: where the log file is on each platform, that the `>>> STARTING` / `<<< COMPLETED` pairs are timestamped, and how to turn the console back on. Never a risk to the MCP server despite that server speaking stdio: it does not call `Initialize`, and neither does the CLI — checked before assuming. |
| B137 | **The commit dialog's diff showed no modified file at all on Linux** | phase 7b-6 2026-09-09 | ⭐⭐⭐ | S | Reported by comparing the Linux build with the Windows Photino one by hand, and it is a **product defect on Linux, not a host difference** — the same build is correct on Windows. `ChangeReview` canonicalised every VCS path on `\` to key its folder tree ("Git uses '/', SVN on Windows uses '\'"), and `FileTreeNode.FullPath` is not only a key: `SelectFile` hands it to `Path.Combine` and `File.Exists` to read the working copy. On Linux `Lib\Thing.mo` is **one file name containing backslashes**, not a relative path, so the file was never found and `_modifiedContent` stayed null. **Only half the view failed, which is why it read as "the file is missing" rather than "the diff is broken"**: the HEAD side still worked, because `GitRevisionControlSystem` normalises `\`→`/` for itself before asking git. Commit and revert were never affected for the same reason — the git layer defends itself at the point of use. **Fixed:** `VcsRelativePath.Canonical` is the one rule, canonicalising on `/` — which is the only choice that works, since git accepts nothing else and Windows accepts `/` as a separator too, so a canonical path can go straight to `Path.Combine` on either platform. The git layer's four hand-written copies of the same conversion now call it. **The test that existed pinned the defect**: `EachFilesFullPathIsKept` asserted `FullPath == @"Lib\Sub\Thing.mo"` — the *shape*, never whether that shape can open the file it names. Replaced by one that writes a real file and asserts `Path.Combine(root, FullPath)` reaches it, from both separator conventions; watched failing against the old code first. Same family as B114 and B118, and the third time a Windows path assumption has survived because the assertion was about a string. |
| B138 | **The About dialog's links were dead on Linux, and phase 7a predicted exactly this** | phase 7b-6 2026-09-09 | ⭐⭐ | S | `OpenGitHub` and `OpenDocumentation` called `JSRuntime.InvokeVoidAsync("open", url, "_blank")`. WebView2 turns `window.open` into a browser launch; **WebKitGTK does nothing with it whatsoever**, and a popup the host declines to create raises nothing, so both buttons were silently inert. 7a's note called it in advance — *"`window.open` in particular behaves differently under WebKitGTK and may need routing through a native shell-open — a phase-7b work item, and a probe in Layer 3"* — and the probe half is no longer available, because the probe set froze when the MAUI baseline was captured. This is the work item arriving without the probe that was meant to catch it. MLQT already had a working mechanism and used it in one place only: `CreatePullRequestDialog` shelled out with `UseShellExecute = true`, which is engine-independent and honours the user's own default browser. Two implementations of one rule, one of which worked. **Fixed:** `ExternalBrowser.Open` is the single way, used by both callers, and refuses anything that is not an absolute `http`/`https` URL — shell-execute opens whatever it is handed, including local paths and executables, and that is worth being deliberate about before a caller stops passing a literal. The launch cannot be tested without opening a browser on the machine running the suite, so the decision is a separate method and that is what has tests. Verified end to end by shimming `xdg-open` onto `PATH` and watching .NET hand it the exact URL — no browser window needed to prove the path works. |
| B139 | **The diff panes scrolled each other back and forth, and on WebKitGTK that made scrolling crawl** | phase 7b-6 2026-09-09 | ⭐⭐ | S | Reported as "vertical scrolling in the side-by-side diff is very slow compared with Windows, and fine in every other tab" — and the tab detail was the diagnosis: `diffViewer.initSyncScroll` is used by that view and nothing else. **Fixed twice, and the first fix is the more useful half of this entry.** The guard was originally `syncing = true; other.scrollTop = this.scrollTop; syncing = false`, which suppresses nothing because scroll events are dispatched asynchronously. Releasing the flag on the next animation frame instead **looked** right and **changed nothing**, and it was shipped on the strength of a simulation that could not have told the difference: it modelled the panes jumping instantly, where the echo carries the same value and is skipped by a value check. The owner tried it and reported the view still slow. **What the too-simple model left out is the entire mechanism.** WebKitGTK scrolls the wheel *smoothly*, over several frames. Within a frame the scroll steps run **before** animation-frame callbacks, so an echo caused in frame N is delivered in frame N+1 — after the flag has been released — and by then the animating pane has moved on, so the echo carries a **stale** position, the values differ, and it is written. Assigning `scrollTop` to a scrolling element cancels its animation, so every wheel tick was cut short. Invisible on WebView2, where the wheel moves instantly and the echo is a no-op. **Fixed properly by ownership**: whichever pane the user is scrolling owns the pair for 150 ms, refreshed on each of its own scroll events, and only the owner drives the other. An echo is by definition not from the owner, so it needs no recognising. Measured against a model that advances an animation frame by frame — **five wheel ticks asking for 265px travelled 93px with five cancellations before, and the full distance with none after**, with the panes staying in sync whichever one is scrolled. That model is committed as `build/diff-scroll-simulation.js` and runs against any revision of the file, because the lesson is not about scrolling: **a simulation that cannot reproduce the defect will happily certify a fix that does nothing**, and this one certified one. |
| B140 | **WebKitGTK's overlay scrollbars sit on top of the controls, and MudBlazor's own injected CSS was disabling every fix** | phase 7b-6 2026-09-09 | ⭐⭐ | S | Reported with a screenshot: the library browser's "more actions" and revert buttons are hard to click because the scrollbar is over them. **Fixed on the third attempt, and the first two are the useful part of this entry — each was reasoned, each looked right, and the owner had to report it still broken.** **Attempt 1 got the diagnosis wrong** ("the bars are wider"). Measuring the engine directly through the WebKit GObject bindings says it is not width: WebKitGTK draws **overlay** scrollbars, which take no layout width and float above the content, so the content is laid out underneath and the bar swallows the clicks. WebView2's are classic and occupy layout. Styling `::-webkit-scrollbar` is the fix because *having any such rule* makes WebKit render a classic, layout-occupying bar. **Attempt 2 disabled itself.** It declared `scrollbar-width`/`scrollbar-color` alongside the pseudo-elements as a hedge — "whichever the engine honours" — and setting either standard property makes WebKit ignore the `-webkit-` rules entirely. Measured: `::-webkit-scrollbar{width:14px}` reserves **14**, the same plus `scrollbar-width: thin` reserves **0**. **Attempt 3 found why the corrected CSS still did nothing, and it was never our CSS.** Verified in an isolated document it reserved 12px; verified in the running application it reserved **0**. The difference is that something in the **MudBlazor stack injects a `<style>` at runtime** carrying `html, body * { scrollbar-color: rgb(196,196,196) transparent; scrollbar-width: thin; }` — the identical hedge, applied to every element, loaded after our stylesheet. It is assembled in code from theme values, so it appears in no shipped static asset and grepping the packages for it finds nothing; it was found by enumerating `document.styleSheets` in the live page. **Fixed** by overriding it — `html, body * { scrollbar-width: auto !important; scrollbar-color: auto !important; }` — with the pseudo-element width also marked `!important`, since the injected sheet declares one too. Verified **in the running application**, not in a test page: 0px reserved before, **12px after**. Three lessons, in order of value: **verify in the real page, because an isolated repro proved the wrong thing twice**; a hedge that declares two mechanisms can disable the one that works; and `document.styleSheets` in the live document finds what grepping the source cannot. |
| B141 | **The Linux development machine renders WebKit in software, so it is not a fair performance reference** | phase 7b-6 2026-09-09 | ⭐ | S | Noted while chasing B139, and it bounds what any Linux-versus-Windows speed comparison from this machine can mean. `systemd-detect-virt` reports **microsoft** — a Hyper-V guest — and while `/dev/dri/card1` exists, Mesa cannot pick a device: the app logs `libEGL warning: failed to get driver name for fd -1` and `MESA: error: ZINK: failed to choose pdev` on every start. WebKit falls back to **software rendering**, so all compositing and painting is on the CPU. 7b-0 recorded the same warnings and correctly called them harmless for *correctness* — Cytoscape draws on a 2D canvas and every probe passes. They are not harmless for *smoothness*, and that distinction was not drawn until a scrolling complaint needed explaining. Layout is not the cost: measured in the engine, 120 forced scroll-and-reflow steps over a 42,000-node diff take **6 ms**, so the DOM size the diff view produces (no virtualisation — every line of both files) is not what makes it feel slow. What is left is paint and composite, which is where software rendering lands. **Open, and not MLQT's to fix**: it is a property of this machine. It matters because it is the machine every Linux judgement in phase 7b has been made on — so "slower than Windows" from here is an upper bound on the problem, not a measurement of it, and a bare-metal Linux box would be the place to confirm anything that still looks slow after B139. **✅ recorded in the product (2026-09-11)** — not fixed, because there is nothing here to fix. Instead the host logs one line on Linux at startup saying whether a DRM render node exists, and when there is none, that the webview paints on the CPU and any rendering measurement from this machine is a lower bound. The next person investigating "MLQT feels slow on Linux" starts from the log rather than from first principles, which is what this cost the first time. `GraphicsEnvironment.Describe` is pure and takes the facts, so the no-render-node case is tested from a machine that has one. |
| B142 | **The `desktop-selftest` job failed on Linux, and was passing on Windows by luck** — ✅ **both green 2026-09-09** | phase 7b-6 2026-09-09 | ⭐⭐ | S | The two 7a leftovers shipped unrehearsed — nothing could run them locally — and the first CI run said Windows passed and Linux failed at the capture step, with webkit and xvfb installed and the publish successful. **Linux: `libnotify.so.4: cannot open shared object file`, then `Aborted (core dumped)`, exit 134.** `Photino.Native.so` lists it among its `NEEDED` entries, it comes from **`libnotify4`**, and `libwebkit2gtk-4.1-0` does not depend on it — so a runner with webkit installed still cannot load Photino. Every other NEEDED library arrives as a webkit dependency, which is why this was the only one missed. Diagnosed from `objdump -p` on the shipped binary before the log was readable, then **confirmed against the log**. Fixed; the install list now comes from the `NEEDED` entries rather than from what seemed necessary. **Windows was worse, and only a guard added for a different reason found it.** The capture step invoked the host directly, and `MLQT.Photino` is a `WinExe`, so **the shell does not block on it**: the step returned while the host was still starting — the log shows WebView2 shutting down five seconds after the step had finished. It passed anyway, because the comparison step that follows takes long enough to build that the report had appeared by the time it was read. **Green by timing.** Had it ever been slower, the comparison would have run against the committed record — a report from another machine on another day — and passed, which is exactly the failure this job exists to prevent. Now `Start-Process -PassThru` and `WaitForExit`. The hole was found because the step was changed to delete the capture first and fail if nothing replaced it. That guard was added on the reasoning that a clean exit with no output would pass silently; it turned out that was not hypothetical, it was what Windows had been doing all along. **Green on both platforms after three runs**, none of which could have been rehearsed beforehand — and each of the two failures was a different kind: one platform could not load the binary at all, the other was reporting success without waiting for it. |
| B144 | **The Photino host shipped no svn client, and the docs promised one** | phase 7b-7 2026-09-09 | ⭐⭐⭐ | S | `MLQT.csproj` copies the SlikSVN payload into its output as `svn/`; **`MLQT.Photino.csproj` did not**. The host was ported through the whole of 7b without it and nothing noticed, because `SvnToolLocator` falls back to `PATH`, every developer machine has svn installed, and the self-test's `svn.client` probe answered `svn.exe` on all of them — a probe that cannot distinguish "we ship this" from "you happen to have it". The shipped article would have had **no SVN at all**, against a `getting-started.md` that says "everything needed is included in the MLQT download". Found by asking what goes into the installer (7b-7), not by any test. **Fixed:** both hosts bundle it, Windows-only, from a payload moved to `svn-tools/` at the repository root so deleting the MAUI project at 7b-8 does not take it with them; on Linux the `.deb` declares `subversion` instead. `BundledSvnClientTests` reads the project files, because the payload is not committed and a build-output check finds nothing on a developer machine. |
| B145 | **Every release shipped a GUI and an MCP server that said 1.0.0** | phase 7b-7 2026-09-10 | ⭐⭐ | S | `release.yml` passed `-p:Version` to the two CLI commands and to nothing else, and `MLQT.Cli.csproj` pinned `0.1.0` of its own — so a tagged release produced a CLI that agreed with the tag, an MCP server left at the SDK default, and a GUI whose version line said **1.0.0**. Measured from the built assembly, not inferred. The MAUI publish did pass `ApplicationDisplayVersion`, which sets the app manifest and not `AssemblyInformationalVersion` — the property `ToolInfo`, every SARIF document and `MainLayout` actually read. **Nothing could have caught it**: the number is only wrong in the published artefact and it is wrong in a way that looks like a plausible version. **Fixed** by `Directory.Build.props` (default `0.0.0-dev`, so an untagged build says something true), removing the pinned version, and passing `-p:Version` from every publish. `ReleaseVersionTests` guards both halves — the workflow, because that is where the omission was, and the assemblies, because a project pinning its own version would satisfy the first and still diverge. |
| B146 | **A self-test that never finishes hangs a release build for six hours** | phase 7 review 2026-09-11 | ⭐⭐ | S | Four places start the GUI to run its 16 /selftest probes and wait for it to exit. The CI job bounds it (`timeout-minutes: 5`, whose comment says exactly why: "a window that never gets that far would otherwise hold the job open for six hours"), and so does the Linux half of the release (`timeout 300 xvfb-run -a mlqt-gui`). **The other two do not**: `publish-tools.ps1` waits on `Start-Process -Wait` with no bound, on both platforms, and the Windows release job's "install it, and run what it installed" step does the same — and no step or job in `release.yml` carries `timeout-minutes` at all. A host that starts and never renders is the failure this exact guard was added for on the CI path, so the shape is known and was not carried across. **Open**, and small: bound the wait where the GUI is started rather than per caller, so the rule has one home. Worth noting the failure is worse in a release than in CI — it burns a runner for the job's full timeout and reports nothing about why. **✅ shipped (2026-09-11)** — bounded where the GUI is started, so the rule has one home per script rather than one per caller: `publish-tools.ps1` takes a `-SelfTestTimeoutSeconds` (300 by default) and kills the host that overruns it, `package-deb.sh` wraps its run in `timeout` the way its MCP handshake already did, and the Windows release step waits the same five minutes its Linux counterpart does. **Both release jobs also carry `timeout-minutes: 60`**, which is the version of this that catches whatever hangs next. `TestRunnerScriptTests` holds all four, and the job-level bound, so losing one is a failing test rather than a six-hour runner. |
| B147 | **The interface cache bought a third of the check time with a fifth of the memory** | phase 7 review 2026-09-11 | ⭐⭐ | S/M | B128's `ClassElementResolver.InterfaceCache` keeps every walked class's extracted interface for the length of a run, which is what took inherited-name resolution from 122 thread-seconds to 6. **What it costs was not measured when it landed.** Measured now, over MSL (6,487 classes): peak working set **614 MB before, 733 MB after** — about 119 MB, or a fifth more, for a third less time. That is a good trade on this library and it is the wrong shape to leave unbounded: the cache grows with the number of classes walked, and the project this phase's performance reports came from holds a **118,612-model graph**, an order of magnitude more. The arithmetic is not linear — a run only walks the checked set and its base classes — but nobody has measured it there, and running out of memory is a worse failure than running slowly. **Open.** Measure on a large project first; if it is material, the fix is to keep only the element names the spell rules actually ask for rather than the whole `ClassInterface`, which is a fraction of the size. **✅ shipped (2026-09-11)** — the cache now keeps **only the classes that get read again**. The saving always came from base classes, which sit on the chain of every class that inherits them; the class a caller asks *about* is walked once per query, so its interface was stored and never read — and a library has far more of those than it has base classes. Measured over MSL: **614 MB before B128, 733 MB with it, 622 MB now**, against 34.0s / ~22s / **22.6s**. The memory is back within 8 MB of never having cached anything and the speed is kept, and the findings are byte-identical across all three (17,784). **It also uncovered a real race, which the first version had been hiding.** Extraction goes through `ModelDefinition.Borrow`, which parses a class if the tree is absent and releases it again afterwards — so two threads extracting the *same* class race, and the loser gets an empty interface. Caching every walked class concealed it, because the second thread read the first one's answer; not caching the query target exposed it immediately, as 3 of 64 parallel resolutions returning nothing. Extraction is now single-flight per class through a `Lazy`, whose entry is dropped again unless it is worth keeping. |
| B148 | **The journeys upload no Playwright trace, which 7a asked for and said why** | phase 7 review 2026-09-11 | ⭐⭐ | S | The 7a plan's CI section is explicit: "Playwright traces on failure (`--trace on-first-retry`) uploaded as an artifact — journey failures on a headless Linux runner are otherwise near-undebuggable." It was never implemented. `ui-journeys` uploads only the `.trx`, which gives an assertion message and a stack — not the DOM, the screenshots or the network log that say *why* a selector did not match on a machine nobody can see. **It is about to matter more than it did**: the journeys now run on two platforms, and `nightly-webkit.yml` runs them under a different browser engine and **has never run at all** (B143), so the first WebKit failure will arrive with the least debuggable evidence of any job here. **Open**, and small: start a trace per journey in the fixture, keep it on failure, upload it beside the trx. **✅ shipped (2026-09-11)** — `TestHostFixture` records a Playwright trace per journey when `MLQT_JOURNEY_TRACE` names a directory, with screenshots, snapshots and sources, one file per test and named after it. Both journey workflows set it and **upload the directory only when the job failed**, so a green run stores nothing and a red one carries the DOM at each step, the console and the network log. Deciding it in the upload rather than in the code means nothing has to know how a test ended. Verified by running the journeys with it set and reading the files back — three traces for the three journeys that open a page, none for the one that drives the pipeline API, and the directory created by the fixture rather than assumed. |
| B149 | **A window restored onto a monitor that is no longer there cannot be reached** | phase 7 review 2026-09-11 | ⭐⭐ | S | `WindowPlacement.IsUsable` accepts any saved position between -10,000 and 20,000 and says so: "deliberately loose... does not try to check it against the current monitor layout, which changes while the application is closed". The consequence is the ordinary laptop case: dock, open MLQT on the second screen, close it, undock — and the next launch restores the window at x=3000 on a single 1920-wide display, where it is invisible and its title bar cannot be grabbed. **Nothing tells the user what happened and nothing documents the way out**, which is to delete `windowPlacement` from `settings.json`. MAUI restored placement itself, so this is a behaviour the host swap took over and made ours. **Open.** Photino exposes the monitor list (`MainMonitor` at minimum), so clamping a restored window onto a visible work area is tractable now in a way the comment assumed it was not — and `WindowGeometry` already has the arithmetic and the tests. **✅ shipped (2026-09-11)** — the remembered placement is checked against the monitor layout once the window exists, which is the first moment Photino will answer. A window with less than 120x30 of itself on any work area is moved to the middle of the first one, **keeping the size the user chose** and only shrinking it to fit — losing a deliberately large window is a poor answer to a problem caused by unplugging a cable. It says so in the log, because a window that quietly appears somewhere else is the kind of thing people doubt their own memory about. The arithmetic is in `WindowGeometry` where it is tested without a display: a screen arranged left of or above the primary is still a screen, a sliver off the edge is not enough to grab, and no monitors at all means nothing to judge against rather than "move everything". |
| B150 | **Neither installer is ever tested doing the thing every user after the first does** | phase 7 review 2026-09-11 | ⭐⭐ | S | Both release jobs prove a great deal about a **first** install: they install what they built, check the tree, run all three tools, run the GUI's 16 probes from where they landed, and uninstall. Neither ever installs **over an existing install**, which is the path every user takes from the second release onwards. It is also where the interesting failures live: files held open by a running MLQT, a `.deb` upgrading in place, and the `AppId` whose comment claims it "is what makes an upgrade replace the previous install instead of sitting beside it" — a claim nothing checks. **Open**, and cheap in the jobs that already exist: install the same artefact twice, the second time over the first, and assert one entry in Apps & Features (or one `dpkg` package) and a working tool afterwards. **✅ shipped (2026-09-11)** — both release jobs now install the artefact a second time over the first, before uninstalling. Windows asserts **exactly one MLQT in Apps & Features**, which is what the `AppId` is for and what nothing had ever checked, and that the tool still runs afterwards; Linux re-installs through `apt --reinstall` and asserts one `mlqt` package in `dpkg`, which is stricter than a file copy — a conffile it cannot merge or a maintainer script that assumed nothing was there fails here. Installing the same artefact twice is not a real version-to-version upgrade, but it exercises what actually goes wrong: files in use, a duplicated registration, a half-written tree. |
| B151 | **A class whose resource analysis throws disappears from the graph in silence** | phase 7 review 2026-09-11 | ⭐ | S | **Pre-existing, not from this branch** (both lines date to the initial commit), found while reviewing phase 7. `GraphBuilder`'s dependency pass wraps each `AddModelUsesModel` in `catch { }`, and its LoadSelector pass-2 wraps a whole class's resource analysis the same way — so a class that throws contributes no resources and no edges, and nothing anywhere records that it happened. This is the exact shape the phase fixed twice in the checking path, each time because the totals moved between runs with nothing to explain it: `LibraryCheckSession` now reports a `CheckFailure` for a class that cannot be checked, and `GraphAnalysisRunner` does the same for an analysis that throws — both with comments saying the failure must not vanish. The graph builder never got the same treatment. **Open**, and the fix is the established one rather than a new idea. **✅ shipped (2026-09-11)** — both dependency passes, in both methods. The edge add is now **asked rather than thrown and caught**: `AddModelUsesModel` throws when either end is missing and a reference to a class outside the loaded set is the ordinary case, so the catch was swallowing normal operation and everything else with it. The LoadSelector pass reports through the `onModelFailed` callback both methods already take, which is what the per-class check and the graph analyses do with the same failure. Tested for the case that matters: a reference to an unloaded class is not a failure, the loaded one beside it still gets its edge, and the scoped pass the app uses after a VCS operation behaves the same. |
| B152 | **Every screenshot in the documentation predates every feature in it** | doc review 2026-09-11 | ⭐⭐ | M | All 53 images under `Documentation/Images` were committed on **2026-03-17**, in the initial public commit, and not one has been touched since. Everything phases 1-6 added is therefore absent from the pictures — the Metrics tab, baseline status on findings, suppression, the coverage trend — and phase 7 replaced the host underneath them, so the window they show is MAUI's. The text is in far better shape than the images: an audit against the code found **every one of the 31 rule ids documented**, every CLI option in `cli.md` (and none it does not accept), and every settable style property in `settings-reference.md`. **The mechanism to regenerate them exists and is proven.** `MLQT.Journeys/DocumentationScreenshots` drives the real components in a real browser through `MLQT.TestHost` — the same apparatus the journeys use — and writes a PNG per tab when `MLQT_DOC_SCREENSHOTS` names a directory. The Settings tab already comes out usable. So the pictures become reproducible, free of personal data and local paths, and regenerable when the UI changes rather than re-taken by hand. **✅ The state setup is done (2026-09-11).** The harness now does what a user does: it writes the repository's `.mlqt/settings.json` with three rules on, creates a project, adds the repository, loads the libraries it discovers, runs the check and selects a class — and the Code Review page comes out showing real findings ("2 Findings to review", `Lib.badly_named` missing a description and not PascalCase) produced by the application's own pipeline from its own configuration. Four steps were needed and each was a fact about the product worth knowing: adding a repository *discovers* libraries but does not load them, loading them does not check them, MLQT ships with every rule off, and in repository mode `MainLayout` renders one `LibraryBrowser` per repository — which is why loading a library through `ILibraryDataService` alone left **every page showing it except the tree** — the Metrics tab reports the 4 classes correctly while the left-hand browser stays blank, so what is missing is whatever a user's project and repository give that panel, not the load itself. Each image still needs its own arrangement: a diff open, a snapshot recorded for the trend, a dialog on screen. **✅ The fixture library is enriched (2026-09-11)**, which was the next limit: four flat classes that referenced nothing and used no files meant the Dependencies tab correctly reported "No models will be affected" and External Resources correctly reported none. `LibraryFixture` now writes fourteen classes over four sub-packages that use each other three levels deep, and every resource reference kind MLQT recognises — a `Bitmap` in an icon, `loadResource` defaults, a `loadSelector` dialog annotation, a `modelica://` image in documentation and the four external-function annotations — with the files they point at written to `Resources/`. One is deliberately absent, because "missing" is a state that page exists to show. The External Resources tab now comes out with its type filters, its tree and a **Missing (1)** warning chip, and the Metrics tab reports 14 classes across package/model/function/connector. Two facts the enrichment taught: resource *edges* are built by the dependency pass while the parse trees are in hand, so `AnalyzeResourcesAsync` must run after `EnsureDependenciesAnalyzedAsync` or it indexes an empty graph; and the tree is lazy, so clicking a node's label selects it and leaves its children out of the DOM entirely — the arrow is what loads them. Transient snackbars have to be waited out or dismissed, or they sit across the picture. **Two kinds of image this cannot produce**, and they are the ones phase 7 changed: anything showing the *window* — title bar, icon, taskbar, a native file dialog — because the test host is a browser, and nothing can drive the Photino window automatically (7a says why: a CDP-driven WebView2 is work that cannot be carried to WebKitGTK). Those stay photographs. Noted alongside: `getting-started-10.png` and `settings-reference-2.png` are on disk and referenced by nothing, so either a section lost its picture or the picture outlived its section. **✅ 41 of the 53 images are regenerated (2026-09-11)**, from the real UI, and the captions that no longer described the picture under them were rewritten with them. `DocumentationScreenshots` now walks the application the way the documentation does: every tab, the close-ups (the toolbars, the repository header rows, the tree with its status chips), both views of the left panel, the Edit Repository Details dialog and three sections of it, the Add Repository dialog in both of its tabs, and eight of the ten Git dialogs. Each is written as the file the markdown already links to, so nothing in the text had to move. **What stays a photograph, and why**: the two Dymola pictures (`code-review-4`, and `code-review-5` — the Finding Details dialog opens only for a finding carrying Details, which a style rule does not produce), the six SVN ones (no server), `settings-reference-4` (the SVN branch directories section is only rendered for an SVN repository), and `git-operations-6`. That last one was **tried and abandoned with something learnt**: the merge dialog's ready-to-merge phase needs a clean working copy, and committing the fixture's change through git at the very end is not enough, because MLQT holds the working-copy status it last read and only a VCS operation *in the application* makes it read again — reasonable behaviour, and no way through from a screenshot run. Two that looked unreachable were reached: `settings-reference-7` (the fixture gained `Lib.Untidy`, an import after a declaration and three public sections, and the repository's settings turned the two layout rules on, so the page reports "does not have its import statements before the rest of the class definition" and "contains more than 1 public section") and `file-monitoring-1` (the monitor is started and three files are edited on disk from outside MLQT, which is what the red badge counts). And anything showing the window frame. **Two fixture facts made the rest possible**: a second commit, so the history is a history and one file in it is modified rather than added; and a second branch, so the switch-branch, merge and pull-request dialogs have somewhere to point. And one path fact: MLQT prints the full path of what it describes, so the fixture repository is written under the shared documents folder for these runs — a temp directory named after a GUID under a developer's profile would have gone into the manual. |
| B153 | **The MCP server grew 22 tools since anything counted them** | doc review 2026-09-11 | ⭐ | S | `Documentation/mcp-server.md` said "60+ tools"; there are **82**. The groups it lists are still right and the full list it points at — `MLQT.McpServer/README.md` — names all but one of them, so this was a number going stale rather than a capability going undocumented. **✅ fixed (2026-09-11)**, along with `MLQT_PHOTINO_LOG`, the desktop host's own logging switch, which nothing told a user about — it is now in `troubleshooting.md` beside `MLQT_LOG_CONSOLE`, with the reason it is silent by default. **Still open: `GetServerInfo` is in neither document.** A tool an agent can call and no document mentions is the shape this backlog keeps naming, and the count above is only correct until the next tool lands — so the durable fix is a test over the tool list, the way `RuleCatalog` is held to its docs. |
| B154 | **A journey fails about one full-suite run in four, always the same one** | ✅ fixed 2026-09-11 | ⭐⭐ | S | `TabNavigationJourney.MovingThroughEveryTabAndBack_RaisesNothing` timed out after 30s clicking a tab, and Playwright reported the element "visible, enabled and stable" while the click never completed. **The trace named it** (B148's first use): the click was intercepted by a `mud-overlay-scrim` belonging to a `mud-dialog-container` — a **modal progress dialog**, covering the tab strip. The dialog is the product working correctly. Opening the **Metrics** tab starts the deferred analysis if any class is loaded (`MetricsDashboard.EnsureAnalysisStarted`), `MainLayout` raises its non-closable "Running style checking" dialog while that runs, and the *next* click — the fifth tab, Settings — lands on the scrim. Nothing a test can do closes it; it closes when the work finishes. **Why it was intermittent and why the suite mattered**: journeys share one host, and `AppState` and `ILibraryDataService` are singletons, so whether the Metrics tab finds anything loaded — and whether `HasStyleCheckingRun` is already set — depends entirely on what ran before. That is also why the journey never failed on its own. **The fix is to wait rather than to force**: `ShellReadiness.WaitUntilClickableAsync` moves the pointer off the strip, waits for any popover to close and then waits for the scrim to go, which is what a user does. Forcing the click would have sailed through a real overlay too, and that is a defect worth failing on. One helper now, in place of the private `DismissTooltipsAsync` that `TabNavigationJourney` and `SettingsJourney` each had: both were right about tooltips and neither knew about dialogs. **Six consecutive full-suite runs green** afterwards, against about one in three or four before; two of the six took twice as long, which is the wait doing its job. |
| B155 | **The revision diff may be comparing against nothing** | doc screenshots 2026-09-11 | ⭐⭐ | S | Found by photographing it. In the VCS History dialog, clicking a file in a commit's changed-files popover opens `Diff: <file> @ <revision>`, which compares that revision against the working copy. Against the **first** commit of a file the fixture later changed, the dialog reports `+7 / -1 lines` and lists the *first* revision's seven lines as additions — the two lines the second commit added are not in it at all, and the working copy is where they live. Either the working-copy side is not being read, or the two sides are the wrong way round; against an unchanged file it says "File is identical", which is right and says nothing about which. **A question, not a diagnosis**: nobody has stepped through `ShowFileDiffAsync` yet. Worth doing before `git-operations-10.png` is trusted as documentation, because that picture is now this behaviour. |
| B156 | **A bUnit test found an element in one render and clicked it in the next** | ✅ fixed 2026-09-11 | ⭐⭐ | S | `DialogResultTests.CreateBranch_Cancelled_ReportsCancelled` failed the **Linux** libraries job and passed on Windows in the same run: `UnknownEventHandlerIdException: there is no event handler with ID '12' associated with the 'onclick' event`. **Not a product defect and not a timing accident to be re-run**: an element found on the test thread carries the event-handler ids of the render it was taken from, and `CreateBranchDialog` re-renders on its own when its background branch load returns. Find outside the dispatcher, act inside it, and the two can disagree — which is what bUnit's own exception text tells you to fix by holding the dispatcher across both. The helpers now find the element *inside* `InvokeAsync`. **The same shape was in `LibraryBrowserLazyLoadTests`**, which had never failed: a lazily loaded tree re-renders when its server data arrives, and its two click helpers found on the test thread too. Fixed with it, because this is the third time in this repository that one rule has lived in two places and only one copy knew something. **Why it surfaced now**: nothing in the commit that failed touched it — the branch's own changes were documentation images and journey code. A slower runner is enough. |
**Allocating ids across branches:** the backlog is a shared counter, and two branches working at
once will both take the next free number — git merges the table without a conflict, because the rows
are on different lines, and the collision is only visible to `MarkdownTableTests`. B134 was allocated
twice that way (Wayland icon on one branch, the missing svn client on the other) and the merge that
joined them was pushed without the tests being run; CI caught it on the next commit. **Run the tests
after a merge that touches the backlog**, and if two ids collide, renumber the one with fewer
references.
...
**Closing them (2026-09-07).** B103 went first, because it is the reason the other two were
reachable: the guard now fails when it cannot find the sources, and asks about the row rather than
about the id. It was watched failing on eleven default cells and six labels before a word of
documentation was changed. Widening its rule-id pattern to allow trailing text then found **two more**
wrong labels — eight, not four — which is the second time in this review that the first number was
the wrong number.
