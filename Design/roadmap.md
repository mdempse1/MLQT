...
| B142 | **The Linux `desktop-selftest` job failed on its first run, and my own job could have passed without proving anything** | phase 7b-6 2026-09-09 | ⭐⭐ | S | The two 7a leftovers shipped unrehearsed — nothing could run them locally — and the first CI run says **Windows passed and Linux failed**, at the capture step, with webkit and xvfb installed and the publish successful. **The cause, from `objdump -p` on the shipped binary rather than from the log** (job logs need admin auth): `Photino.Native.so` lists `libnotify.so.4` among its `NEEDED` entries, that comes from **`libnotify4`**, and `libwebkit2gtk-4.1-0` **does not depend on it** — so a runner with webkit installed still cannot load Photino. Everything else it needs arrives as a webkit dependency, which is why only this one was missed. Now installed explicitly, from the `NEEDED` list rather than by guesswork. **Unconfirmed against the actual log** until someone with admin rights reads it. **And a hole in the job itself, found while fixing that**: the capture step wrote over the committed record, so a host that exited cleanly having written *nothing* would leave yesterday's file in place and the comparison would pass — green, from a record taken on another machine on another day, which is precisely the failure the job exists to prevent. It now deletes the capture first and fails if nothing replaced it, on both platforms. The step also prints the loader error and the application log when it fails, because "process exited 134" cannot distinguish a missing shared library from a host that started and never rendered — the same lesson as B133, where the loud failure that replaced a silent one was itself silent. |
