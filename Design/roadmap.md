...
| B120 | **The test host asks for an `app.css` it does not have** | phase 7b-A 2026-09-08 | ⭐ | S | `HostAssetManifest` lists `app.css` — each host's own stylesheet, as distinct from `_content/MLQT.Shared/app.css` — and `MLQT.TestHost` has no `wwwroot` at all, so every page load 404s on it. Harmless there (the test host has no styling of its own to lose) and invisible, because `OpenAndBrowseJourney` only inspects URLs containing `_content/`. Found by dumping a panel's text while chasing a journey selector, in the server log rather than by any test. **Open**, and it matters for **7b-2**: the Photino host is generated from the same manifest, so it will ask for the same file, and there the missing stylesheet is the application's own. Worth widening the journey's 404 check to every same-origin request at the same time — the narrow filter is what hid this. |
