...
being the GNOME dock icon (**B134**), which like B132 is not a defect in the process at all — a desktop
shell identifies an application by an installed entry, so both belong to packaging. 7a's two leftovers
...
| B134 | **The GNOME dock showed a generic icon, and again nothing in MLQT was wrong** | phase 7b-6 2026-09-09 | ⭐⭐ | S | The Windows taskbar finding (B132) on a different mechanism, found by the Linux manual checklist. Title bar and Alt-Tab carried the MLQT icon; the dash and dock did not. `SetIconFile(mlqt-256.png)` works and is not the cause — **GNOME Shell takes a dock icon from the matching desktop entry, keyed on the Wayland `app_id`**, and none was installed, so there was nothing to read. The `app_id` was **observed rather than inferred**, which is the one thing B132 cost a day to learn: `WAYLAND_DEBUG=1 ./MLQT.Photino` shows `-> xdg_toplevel#43.set_app_id("MLQT.Photino")`, a native Wayland toplevel whose id is the executable's file name (GTK's `g_get_prgname()`). Installing `~/.local/share/applications/MLQT.Photino.desktop` with a matching `Icon=` and `StartupWMClass=MLQT.Photino`, then restarting, **made the icon appear** — so the fix is demonstrated, not proposed. The test file has been removed because its `Exec` pointed at a temporary publish. **7b-7 owns installing a real one**, with `Icon=` naming a `hicolor` theme icon rather than an absolute path, which is right for a tarball and wrong for a `.deb`. The recipe is in `Branding/README.md` beside the Windows one. The general lesson is now stated twice from two operating systems: **a desktop shell identifies an application by an installed entry, not by the window**, so this class of defect is never in the process. |
