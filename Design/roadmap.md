...
| B140 | **WebKitGTK's overlay scrollbars sit on top of the controls, and the first fix disabled itself** | phase 7b-6 2026-09-09 | ⭐⭐ | S | Reported with a screenshot: the library browser's "more actions" and revert buttons are hard to click because the scrollbar is over them. Recorded first as "the bars are wider and look different", which was **the wrong diagnosis** — and the right one only came from measuring the engine instead of reading the screenshot. **It is not width, it is overlay.** WebKitGTK draws overlay scrollbars: they take **no layout width** and float above the content, so the content is laid out underneath and the bar swallows clicks meant for it. WebView2's are classic and occupy layout, so nothing is ever under one. Measured directly against `libwebkit2gtk-4.1` — the same library Photino loads — through the GObject bindings, as the gutter a scrollable div reserves. **The first attempt declared both `scrollbar-width`/`scrollbar-color` and the `::-webkit-scrollbar` pseudo-elements, as a hedge — and the hedge is exactly what broke it.** Setting either standard property makes WebKit ignore the `-webkit-` pseudo-elements, and WebKitGTK's own implementation of the standard ones is the overlay bar we were trying to leave. The measurement is unambiguous: `::-webkit-scrollbar { width: 14px }` reserves **14**; the same rule plus `scrollbar-width: thin` reserves **0**; plus `scrollbar-color` alone, **0**; nothing at all, **0**. **Fixed** by styling only `::-webkit-scrollbar`, which makes WebKit render a classic layout-occupying bar — the width it sets is almost beside the point, it is *having a rule at all* that stops it being an overlay. Chromium applies the same precedence, so one mechanism covers both hosts. Verified against the real `app.css` in the real engine: **12px reserved, content laid out beside the bar rather than under it.** The lesson is the hedge: "declare both, whichever the engine honours" reads as cautious and was the defect. |
