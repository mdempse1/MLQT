...
| B139 | **The diff panes scrolled each other back and forth, and on WebKitGTK that made scrolling crawl** | phase 7b-6 2026-09-09 | ⭐⭐ | S | Reported as "vertical scrolling in the side-by-side diff is very slow compared with Windows, and fine in every other tab" — and the tab detail is the diagnosis: `diffViewer.initSyncScroll` is used by that view and nothing else. The guard was written the obvious way — `syncing = true; other.scrollTop = this.scrollTop; syncing = false;` — and **it suppresses nothing**. Scroll events are dispatched asynchronously, queued and delivered during the rendering step rather than synchronously from the assignment, so the flag is false again by the time the other pane's handler runs. Each tick echoed back and forth. **Invisible on WebView2, crippling on WebKitGTK**: WebKit scrolls the wheel smoothly over several frames, and assigning `scrollTop` to a scrolling element cancels that animation — so every wheel tick started an animation and had it cancelled a frame or two in. A few pixels per tick, in a view whose whole purpose is reading down a file. **Measured rather than argued**: a stand-in for the two panes, in which setting `scrollTop` queues an async scroll event as a browser does, counts the writes to the pane the user is scrolling. Old code: **6 writes for one wheel tick**, and still going. New code: **1**. **Fixed** by releasing the flag on the next animation frame, by which time the echoed event has been delivered and ignored, and by writing only when the value actually differs — a redundant write is still a write, and still cancels an animation. |
| B140 | **Nothing styled the scrollbars, so each engine drew its own and they crowd the controls** | phase 7b-6 2026-09-09 | ⭐ | S | No `::-webkit-scrollbar` or `scrollbar-*` rule existed anywhere in the application, so both hosts fell back to the engine default and the two do not resemble each other: WebView2 draws Chromium's slim overlay bar, WebKitGTK a wider GTK-style one that takes layout width and sits hard against the content. **The impact is not cosmetic**: in the library browser it leaves the "more actions" and revert buttons pressed up against the bar and awkward to hit. Reported with a screenshot. **Fixed** in `app.css`, declaring *both* mechanisms — the standard `scrollbar-width`/`scrollbar-color` and the older `::-webkit-scrollbar` pseudo-elements that Chromium also implements — because which one a given engine honours is not something to guess at, and declaring both costs nothing. The thumb carries a transparent border with `background-clip: content-box`, which insets it from the track and is the part that gives the adjacent controls their few pixels back. **Confirmed by the owner rather than by us**: GNOME refuses programmatic screenshots to an unprivileged caller, so nothing here can look at its own rendering — the same limit 7b-0 and 7b-6 both record. |
