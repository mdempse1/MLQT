...
| B132 | **The Photino window had no taskbar icon** | phase 7b-5 2026-09-08 | ⭐ | S | `ApplicationIcon` puts the icon on the `.exe` — Explorer and Alt-Tab were right — but **Photino creates its own native window and that window has no icon unless one is set on it**, so the taskbar entry stayed generic. Fixed by setting it at runtime through `SetIconFile`, per platform (Windows `.ico`, GTK PNG), with both copied beside the executable because that API takes a path rather than a resource. **Then reported still wrong on the taskbar while right in the title bar and in Explorer** — three places, three sources. Measured: the window carries a 16x16 `ICON_SMALL` and a 32x32 `ICON_BIG`, both correct, so the window was never the problem. A taskbar button is grouped by **Application User Model ID**, and an application declaring none is given whatever the shell derives from the process — `dotnet.exe` under `dotnet run`, or a cached entry for the path. `Program.ClaimTaskbarIdentity` now declares `MLQTProject.MLQT` before the first window exists, which also makes a pin survive the executable moving. Assets in `Branding/`. |
