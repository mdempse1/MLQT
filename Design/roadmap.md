...
| B125 | **Photino felt slower than MAUI; the logs say most of it is not the host** | phase 7b-5 2026-09-08 | ⭐⭐⭐ | M | Three slow things were reported. The application log had already recorded every one of them, on the same library, across both hosts, for weeks — **it was never read**, and two of the three turned out not to be about Photino at all. Dependency analysis over the same 39,860-model graph: MAUI 88.7–163.8 s over eleven runs, Photino **83.1 s and 147.6 s** — the fastest run ever recorded is a Photino one, so there is **no regression to find here**. The style check on the same repository: MAUI 324/325 s on 7 Sep against Photino 420 s on 8 Sep (+30%, matching the report) — but MAUI itself ranged 183–468 s on 2 Sep, so **one sample cannot separate a host cost from a variance that is already ±50%**. Open, and what it needs is a back-to-back A/B on one day rather than more reasoning. B126 and B127 are the two that were diagnosed and fixed; B128 is the much larger effect the series exposed. |
...
| B128 | **The style check on one library has tripled since late August, on MAUI** | phase 7b-5 2026-09-08 | ⭐⭐⭐ | M | The series that came out of B125's log analysis, all of it MAUI and all on the same repository: **~105 s** median over 24–27 Aug, ~190 s on 1–2 Sep, **324/325 s** on 7 Sep. That is a bigger effect than anything the host migration could account for and it predates it. Candidates, in the order they arrived: the phase-5 suppression reads, the phase-6 whole-graph analyses and coverage measurement, and rules the repository may simply have had enabled since. Not yet separated — and worth separating before any more effort goes into the host question, because a 3× is where the time actually is. |
