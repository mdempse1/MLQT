...
| B132 | **The Photino window's taskbar button shows the generic application icon** | phase 7b-5 2026-09-08 | ⭐ | S | **Open, and cosmetic.** Explorer, Alt-Tab and the title bar are all correct; only the taskbar button is not. Everything measurable about the application is right: the window carries `ICON_SMALL` and `ICON_BIG` at the DPI-scaled sizes (20x20 and 40x40 at 125%), `mlqt.ico` holds 16/24/32/48/256, and `MLQT.Photino.exe` embeds exactly one `RT_GROUP_ICON` at `#32512` — byte-identical resources to a probe that **does** show the icon. Ruled out by experiment, each with a screenshot of the taskbar: the icon cache (a reboot), an explicit `AppUserModelID` (tested both ways on a probe using the same id), setting the icon after creation, `WS_EX_APPWINDOW`, destroying and recreating the taskbar button by toggling `WS_EX_TOOLWINDOW`, an application manifest with `supportedOS` and per-monitor DPI, and a saturated UI thread. A **WinForms** window with the same icon file works; a bare **Photino** window works too, once its executable embeds the icon. So the remaining difference is something about the `MLQT.Photino` process specifically, and the next experiment is to build the probe on `PhotinoBlazorApp` rather than a bare `PhotinoWindow` — the last untested structural difference. Two changes made along the way are kept because each is correct on its own terms: the explicit AUMID (a pin now survives the executable moving) and `WindowIcon.Apply` (the window gets 40x40 rather than a stretched 32x32). |
