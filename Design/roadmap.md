...
| B122 | **MAUI's `Preferences` store is in none of the places it is supposed to be** | phase 7b-3 2026-09-08 | ⭐ | S | Looking for it to write the settings migration: not under `HKCU\Software`, not in a WinRT `settings.dat` container, not in `%LocalAppData%\MLQT`, and a registry search for the `ApplicationId` and for a known key name found nothing. MLQT is an unpackaged WinUI app, where `ApplicationData.Current` does not behave as a packaged app's does. **Not fixed and deliberately not chased** — the migration was designed so the answer does not matter: the MAUI app seeds the shared store through its own `Preferences` API, so nothing needs to know where that API keeps things. Recorded because the next person to go looking should know it is not a five-minute lookup, and because any design that *does* need the location is built on sand. |
