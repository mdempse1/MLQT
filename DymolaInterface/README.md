...
- `SetOfflineMode(bool)` - `true` holds every command back until `SetOfflineMode(false)` or a successful `StartDymolaProcessAsync`, even while a Dymola is answering: unlike the offline state the constructor finds, the recovery probe does not undo it. `StopDymolaProcessAsync` sets it too, so a later command cannot reconnect to some other Dymola on the same port
- `LastOutcome` - What became of the last command: `Answered`, or `Offline`, `TimedOut`, `Cancelled` or `Failed`. Every command returns `false`, `null` or an empty string when it gets no result, so a `false` is Dymola's own answer only when this says `Answered`. After `TimedOut`, Dymola is still working on the command and will not answer anything else until it finishes
