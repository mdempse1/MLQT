...
| **IRepositoryService** | Git/SVN repository management, library discovery, VCS operations. `GetWorkingCopyChanges` runs **one status query per repository at a time**, shared by every caller that arrives while it runs (B293) — never call the VCS for status yourself |
